// server.js - GMAIL ENTEGRASYONU - NİHAİ SÜRÜM

require('dotenv').config();
const express = require('express');
// ... (diğer require'lar aynı)

const app = express();
// ... (geri kalan kodun başı aynı)
...
async function schedule_reminder(noteName, time) { /* ...içerik aynı... */ }

// DEĞİŞTİ: Bu fonksiyon artık Gmail kullanıyor
async function send_email(to, subject, body) {
    console.log(`Gmail üzerinden e-posta gönderiliyor. Alıcı: ${to}`);
    try {
        const user = process.env.GMAIL_USER;
        const pass = process.env.GMAIL_APP_PASSWORD;

        if (!user || !pass) {
            throw new Error("Gmail kullanıcı adı veya Uygulama Şifresi .env dosyasında eksik.");
        }

        // Nodemailer'ı Gmail için yapılandırıyoruz
        let transporter = nodemailer.createTransport({
            host: "smtp.gmail.com",
            port: 465,
            secure: true, // true for 465, false for other ports
            auth: {
                user: user, // .env dosyasından gelen Gmail adresin
                pass: pass, // .env dosyasından gelen 16 haneli Uygulama Şifren
            },
        });

        let info = await transporter.sendMail({
            from: `"Coopa Asistan" <${user}>`, // Gönderen olarak kendi Gmail adresin görünecek
            to: to,
            subject: subject,
            text: body,
            html: `<b>${body}</b>`
        ...
        console.log("✉️ E-posta başarıyla gönderildi. Message ID: %s", info.messageId);
        // Artık Ethereal linki yok, çünkü e-posta doğrudan hedefe gitti.
        return { success: true, message: `E-posta başarıyla ${to} adresine gönderildi.` };
    } catch (error) {
        console.error("❌ HATA: Gmail ile e-posta gönderilirken hata oluştu:", error);
        return { success: false, error: error.message };
    }
}


// ... (server.js dosyasının geri kalanı öncekiyle tamamen aynı)

// --- KODUN TAMAMI (KOPYALA-YAPIŞTIR İÇİN) ---
...
async function schedule_reminder(noteName, time) { console.log(`⏰ Yeni hatırlatıcı kuruluyor: Not='${noteName}', Zaman='${time}'`); const [hour, minute] = time.split(':'); if (isNaN(hour) || isNaN(minute) || hour < 0 || hour > 23 || minute < 0 || minute > 59) { return { success: false, message: "Geçersiz zaman formatı. Lütfen 'HH:MM' formatında belirtin." }; } const cronTime = `${minute} ${hour} * * *`; cron.schedule(cronTime, async () => { console.log(`🔔 Hatırlatıcı tetiklendi: '${noteName}' notu gönderiliyor.`); const userEmail = process.env.MY_EMAIL_ADDRESS; if (!userEmail) { console.error("❌ HATA: Hatırlatıcı e-postası gönderilemedi. Lütfen .env dosyanıza MY_EMAIL_ADDRESS değişkenini ekleyin."); return; } const note = await get_note(noteName); if (note.success) { const subject = `Coopa Hatırlatıcısı: ${noteName}`; const body = `Merhaba, bu sana "${noteName}" notun için bir hatırlatma:\n\n---\n${note.content}\n---`; await send_email(userEmail, subject, body); } else { console.error(`❌ HATA: Hatırlatıcı e-postası gönderilemedi. '${noteName}' notu veritabanında bulunamadı.`); } }, { scheduled: true, timezone: "Europe/Istanbul" }); return { success: true, message: `Tamamdır, "${noteName}" notunu sana Türkiye saatiyle ${time}'da hatırlatacağım.` }; }
...
app.post('/execute-action', async (req, res) => { try { const { actionName, to, subject, body, history } = req.body; if (actionName !== 'send_email') throw new Error("Desteklenmeyen eylem."); console.log(`\n👍 Onay alındı! Eylem gerçekleştiriliyor: ${actionName}`); const toolResult = await send_email(to, subject, body); let historyForThisTurn = [...(history || [])]; historyForThisTurn.push({ role: "function", parts: [{ functionResponse: { name: actionName, response: toolResult } }] }); const finalResponseText = toolResult.success ? `E-posta başarıyla gönderildi.` : `E-posta gönderilemedi. Hata: ${toolResult.error}`; historyForThisTurn.push({ role: "model", parts: [{ text: finalResponseText }] }); coopaCore.uploadToIrys(`ONAYLANDI: E-posta gönderildi. Alıcı: ${to}`); res.json({ history: historyForThisTurn }); } catch (error) { console.error("❌ /execute-action rotasında hata:", error); res.status(500).json({ error: "Eylem gerçekleştirilirken bir hata oluştu: " + error.message }); } });
app.listen(port, () => { console.log(`\n✅ Sunucu (Gmail Aktif) başarıyla başlatıldı!`); console.log(`   http://localhost:${port} adresinden erişebilirsiniz.`); });