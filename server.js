// server.js - TÜM FONKSİYONLARI İÇEREN VE HATALARI DÜZELTİLMİŞ NİHAİ SÜRÜM
...
async function create_calendar_event(title, date, time, description = '') {
    try {
        const tokens = await new Promise((resolve, reject) => { db.get(`SELECT * FROM google_auth WHERE id = 1`, (err, row) => { if (err) reject(err); resolve(row); }); });
        if (!tokens || !tokens.refresh_token) { throw new Error("Google kimlik doğrulaması bulunamadı. Lütfen arayüzden yetki verin."); }
        oauth2Client.setCredentials({ refresh_token: tokens.refresh_token });
        const calendar = google.calendar({ version: 'v3', auth: oauth2Client });
        let eventDateStr;
        if (!date || date.toLowerCase() === 'bugün') { const today = new Date(); eventDateStr = `${today.getFullYear()}-${String(today.getMonth() + 1).padStart(2, '0')}-${String(today.getDate()).padStart(2, '0')}`; } else { eventDateStr = date; }
        const eventDateTime = new Date(`${eventDateStr}T${time}`);
        if (isNaN(eventDateTime.getTime())) { throw new Error(`Geçersiz tarih/saat formatı.`); }
        ...
        return { success: true, message: `"${title}" etkinliği takviminize eklendi.`, event_link: response.data.htmlLink };
    } catch (error) {
        console.error("❌ Google Takvim hatası:", error.message);
        return { success: false, error: `Takvim etkinliği oluşturulamadı: ${error.message}` };
    }
}
...
async function find_memory(searchText, userAddress) { return new Promise((resolve) => { const sql = `SELECT irys_id, description, media_type FROM memories WHERE user_address = ? AND (description LIKE ? OR irys_id LIKE ?) ORDER BY id DESC LIMIT 1`; const searchTerm = `%${searchText}%`; console.log(`[DB-DEBUG] ${userAddress} kullanıcısı için anı aranıyor: "${searchText}"`); db.get(sql, [userAddress, searchTerm, searchTerm], (err, row) => { if (err) { console.error("Anı aranırken veritabanı hatası:", err.message); return resolve({ success: false, error: err.message }); } if (row) { console.log("✅ Anı bulundu:", row); resolve({ success: true, message: `"${row.description}" açıklamalı anı bulundu.`, data: { irysId: row.irys_id, description: row.description, mediaType: row.media_type } }); } else { console.log(`"${searchText}" ile eşleşen anı bulunamadı.`); resolve({ success: false, message: `"${searchText}" ile eşleşen bir anı bulunamadı.` }); } }); }); }
