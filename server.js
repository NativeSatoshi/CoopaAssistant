// server.js - PAYLOAD HATASI ÇÖZÜLMÜŞ, TAM VE NİHAİ SÜRÜM
...
async function verifySignature(message, signature, expectedAddress) { try { const recoveredAddress = ethers.verifyMessage(message, signature); return recoveredAddress.toLowerCase() === expectedAddress.toLowerCase(); } catch (error) { console.error("İmza doğrulanırken hata:", error); return false; } }
...
async function get_note(noteName) { return new Promise((resolve) => { const sql = `SELECT content FROM notes WHERE name = ?`; db.get(sql, [noteName], (err, row) => { if (err) { resolve(`"${noteName}" notunu okurken bir veritabanı hatası oluştu: ${err.message}`); } else if (row) { resolve(row.content || "(notun içeriği boş)"); } else { resolve(`"${noteName}" isminde bir not bulunamadı.`); } }); }); }
async function edit_note(noteName, newContent) { const existingContent = await get_note(noteName); if (existingContent.includes("bulunamadı") || existingContent.includes("hatası oluştu")) { return { success: false, message: existingContent }; } const updatedContent = (existingContent && existingContent !== "(notun içeriği boş)") ? existingContent + ", " + newContent : newContent; return create_note(noteName, updatedContent); }
...
async function saveMemory(irysId, description, mediaType, userAddress) { return new Promise((resolve, reject) => { const sql = `INSERT INTO memories (irys_id, description, media_type, user_address) VALUES (?, ?, ?, ?)`; db.run(sql, [irysId, description, mediaType, userAddress], function (err) { if (err) { console.error("Veritabanına anı kaydedilirken hata:", err.message); return reject(err); } console.log(`✅ ${userAddress} adresli kullanıcı için anı veritabanına kaydedildi.`); resolve({ id: this.lastID }); }); });}
async function find_memory(searchText, userAddress, signature) { return new Promise(async (resolve) => { const sql = `SELECT irys_id, description, media_type FROM memories WHERE user_address = ? AND (description LIKE ?) ORDER BY id DESC LIMIT 1`; const searchTerm = `%${searchText}%`; db.get(sql, [userAddress, searchTerm], async (err, row) => { if (err) { return resolve({ success: false, error: err.message }); } if (row) { try { const gatewayUrl = `https://gateway.irys.xyz/${row.irys_id}`; const response = await axios.get(gatewayUrl, { responseType: 'arraybuffer' }); const encryptedBuffer = Buffer.from(response.data, 'binary'); const decryptedBuffer = decryptBuffer(encryptedBuffer, signature); const decryptedDataUrl = `data:${row.media_type};base64,${decryptedBuffer.toString('base64')}`; resolve({ success: true, message: `"${row.description}" açıklamalı anı bulundu.`, data: { decryptedDataUrl, description: row.description } }); } catch (error) { console.error("Anı deşifre edilirken hata:", error); resolve({ success: false, message: "Anı bulundu ancak deşifre edilemedi."}); } } else { resolve({ success: false, message: `"${searchText}" ile eşleşen bir anı bulunamadı.` }); } }); }); }
...
app.get('/auth/google/callback', async (req, res) => { try { const { code } = req.query; const { tokens } = await oauth2Client.getToken(code); const sql = `INSERT OR REPLACE INTO google_auth (id, access_token, refresh_token, expiry_date, scope) VALUES (1, ?, ?, ?, ?)`; db.run(sql, [tokens.access_token, tokens.refresh_token, tokens.expiry_date, tokens.scope]); res.redirect('/?auth=success'); } catch (error) { console.error("Google Auth Callback Hatası:", error.message); res.redirect('/?auth=error'); }});
app.post('/upload', upload.single('memoryFile'), async (req, res) => { try { const { description, userAddress, signature } = req.body; const file = req.file; if (!file || !userAddress || !signature) { return res.status(400).send("Eksik bilgi: Dosya, adres veya imza belirtilmemiş."); } if (!await verifySignature(fixedSignMessage, signature, userAddress)) { return res.status(401).send("Geçersiz imza. Kimlik doğrulanamadı."); } const encryptedBuffer = encryptBuffer(file.buffer, signature); const tags = [{ name: "Content-Type", value: "text/plain" }]; const receipt = await coopaCore.uploadFileToIrys(encryptedBuffer, tags); if (!receipt) { throw new Error("Irys'e şifreli yükleme başarısız oldu."); } await saveMemory(receipt.id, description, file.mimetype, userAddress); const gatewayUrl = `https://gateway.irys.xyz/${receipt.id}`; res.send(`<div style="font-family: sans-serif; padding: 20px;"><h1>✅ Anı Başarıyla Şifrelenerek Kalıcı Olarak Kaydedildi!</h1><p><b>Açıklama:</b> ${description}</p><p><b>Irys İşlem ID:</b> ${receipt.id}</p><p><a href="${gatewayUrl}" target="_blank">Kaydedilen Şifreli Dosyayı Irys'te Görüntüle</a></p><br><a href="/">Sohbete Geri Dön</a></div>`); } catch (error) { console.error("Dosya yükleme sürecinde hata:", error); res.status(500).send(`Bir hata oluştu: ${error.message}`); }});

app.post('/generate', async (req, res) => {
    try {
        const { prompt, history, userAddress, signature } = req.body;
        if (!userAddress || !signature) { throw new Error("İstekle birlikte kullanıcı adresi veya imza gönderilmedi."); }
        if (!await verifySignature(fixedSignMessage, signature, userAddress)) { throw new Error("Geçersiz imza. Kimlik doğrulanamadı."); }
        ...
        while (true) {
            ...
            if (part.functionCall && part.functionCall.name === 'find_memory') {
                ...
                const toolResult = await find_memory(part.functionCall.args.searchText, userAddress, signature);
        ...
        const errHistory = [...(req.body.history || []), { role: "user", parts: [{ text: req.body.prompt }] }, { role: "model", parts: [{ text: `Bir hata oluştu: Sunucu hatası` }] }];
        res.status(500).json({ history: errHistory });
    }
});
app.post('/execute-action', async (req, res) => { /* ... Öncekiyle aynı ... */ });
