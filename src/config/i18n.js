    ...
    tr: [
        {
            functionDeclarations: [
                ...
                { name: "schedule_task", description: "Bir e-postayı, notu veya dosyayı GELECEKTEKİ belirli bir saat için planlar/zamanlar. Kullanıcı 'saat', 'sonra', 'akşam' gibi bir zaman ifadesi kullanırsa MUTLAKA bu araç seçilmelidir.", parameters: { type: "OBJECT", properties: { time: { type: "STRING", description: "Görevin zamanı, MUTLAKA 24 saatlik 'HH:MM' formatında olmalı (örnek: '17:45')." }, noteName: { type: "STRING", description: "E-posta ile gönderilecek, önceden kaydedilmiş bir notun adı." }, subject: { type: "STRING", description: "Sıfırdan oluşturulacak bir e-postanın konusu." }, body: { type: "STRING", description: "Sıfırdan oluşturulacak bir e-postanın içeriği." }, attachmentDescription: { type: "STRING", description: "E-postaya eklenecek olan, daha önce kaydedilmiş bir anının (dosyanın) açıklaması." } }, required: ["time"] } },
                ...
                { name: "create_calendar_event", description: "Kullanıcının takvimine yeni bir etkinlik veya randevu ekler.", parameters: { type: "OBJECT", properties: { title: { type: "STRING" }, date: { type: "STRING", description: "Etkinlik tarihi, 'YYYY-MM-DD' formatında OLMALI. Eğer kullanıcı 'bugün', 'yarın' gibi bir ifade kullanırsa, ilgili tarihi bu formatta KENDİN HESAPLA. Tarih belirtilmezse bugünü kullan." }, time: { type: "STRING", description: "Etkinlik saati, MUTLAKA 24 saatlik 'HH:MM' formatında olmalı (örnek: '14:30')." }, description: { type: "STRING" } }, required: ["title", "time"] } },
                ...
                { name: "get_crypto_price", description: "Bir dijital para biriminin (kripto para) anlık fiyatını belirli bir para birimi cinsinden alır.", parameters: { type: "OBJECT", properties: { coin_name: { type: "STRING", description: "Fiyatı alınacak kripto paranın adı (örnek: 'bitcoin', 'ethereum')." }, currency: { type: "STRING", description: "Fiyatın hangi para biriminde istendiği (örnek: 'usd', 'eur', 'try')." } }, required: ["coin_name", "currency"] } },
                { name: "get_stock_price", description: "Bir şirketin hisse senedinin anlık borsa fiyatını alır.", parameters: { type: "OBJECT", properties: { stock_symbol: { type: "STRING", description: "Fiyatı alınacak hisse senedinin borsa sembolü (örnek: 'AAPL', 'TSLA', 'GOOGL')." } }, required: ["stock_symbol"] } },
                { name: "web_search", description: "Belirli bir konu hakkında internette arama yapar ve güncel sonuçları bulur. Uçak bileti, maç bileti, otel fiyatları veya genel bilgi gibi konular için kullanılır.", parameters: { type: "OBJECT", properties: { query: { type: "STRING", description: "İnternette aranacak olan sorgu (örnek: 'İstanbul'daki en iyi İtalyan restoranları')." } }, required: ["query"] } },
                { name: "record_action_receipt", description: "Kullanıcı adına başarıyla tamamlanan önemli bir eylemi (uçuş bulma, hisse senedi sorgulama, rezervasyon yapma vb.) gelecekte başvurmak üzere bir 'makbuz' olarak kalıcı şekilde kaydeder.", parameters: { type: "OBJECT", properties: { summary: { type: "STRING", description: "Eylemin kısa ve anlaşılır bir özeti (örnek: 'Paris'e Uçak Bileti Bulundu', 'AAPL Hisse Fiyatı Sorgulandı')." }, action_type: { type: "STRING", description: "Eylemin türü (örnek: 'flight_search', 'stock_price_query', 'hotel_booking')." }, details: { type: "OBJECT", description: "Eylemin tüm detaylarını içeren bir JSON nesnesi. Örneğin bir uçuş için: { origin, destination, date, price }." } }, required: ["summary", "action_type", "details"] } },
                { name: "find_receipts", description: "Kullanıcının geçmişte yaptığı ve kaydedilmiş eylemleri (makbuzları) belirli bir tarih aralığı için bulur.", parameters: { type: "OBJECT", properties: { start_date: { type: "STRING", description: "Aramanın başlangıç tarihi. 'YYYY-MM-DD' formatında veya 'bugün', 'dün' gibi göreceli bir ifade olabilir." }, end_date: { type: "STRING", description: "Aramanın bitiş tarihi. 'YYYY-MM-DD' formatında veya 'bugün' gibi göreceli bir ifade olabilir." } }, required: ["start_date", "end_date"] } }
            ]
        }
    ],
    en: [
        {
            functionDeclarations: [
                ...
                { name: "schedule_task", description: "Schedules an email, note, or file for a specific time in the FUTURE. This tool MUST be chosen if the user uses a time expression like 'hour', 'later', 'evening'.", parameters: { type: "OBJECT", properties: { time: { type: "STRING", description: "The time of the task, MUST be in 24-hour 'HH:MM' format (example: '17:45')." }, noteName: { type: "STRING", description: "The name of a pre-saved note to be sent via email." }, subject: { type: "STRING", description: "The subject of an email to be created from scratch." }, body: { type: "STRING", description: "The content of an email to be created from scratch." }, attachmentDescription: { type: "STRING", description: "The description of a previously saved memory (file) to be attached to the email." } }, required: ["time"] } },
                ...
                { name: "create_calendar_event", description: "Adds a new event or appointment to the user's calendar.", parameters: { type: "OBJECT", properties: { title: { type: "STRING" }, date: { type: "STRING", description: "Event date MUST be in 'YYYY-MM-DD' format. If the user says 'today' or 'tomorrow', CALCULATE the corresponding date in this format YOURSELF. If no date is specified, use today." }, time: { type: "STRING", description: "Event time MUST be in 24-hour 'HH:MM' format (example: '14:30')." }, description: { type: "STRING" } }, required: ["title", "time"] } },
                ...
                { name: "get_crypto_price", description: "Gets the current price of a digital currency (cryptocurrency) in a specific currency.", parameters: { type: "OBJECT", properties: { coin_name: { type: "STRING", description: "The name of the cryptocurrency to get the price of (e.g., 'bitcoin', 'ethereum')." }, currency: { type: "STRING", description: "The currency in which the price is requested (e.g., 'usd', 'eur', 'try')." } }, required: ["coin_name", "currency"] } },
                { name: "get_stock_price", description: "Gets the current stock market price of a company's stock.", parameters: { type: "OBJECT", properties: { stock_symbol: { type: "STRING", description: "The stock market symbol of the stock to get the price of (e.g., 'AAPL', 'TSLA', 'GOOGL')." } }, required: ["stock_symbol"] } },
                { name: "web_search", description: "Searches the internet for a specific topic and finds current results. Used for topics like flight tickets, match tickets, hotel prices, or general information.", parameters: { type: "OBJECT", properties: { query: { type: "STRING", description: "The query to be searched on the internet (example: 'best Italian restaurants in Istanbul')." } }, required: ["query"] } },
                { name: "record_action_receipt", description: "Permanently saves an important action successfully completed on behalf of the user (e.g., finding flights, querying stock prices, making a reservation) as a 'receipt' for future reference.", parameters: { type: "OBJECT", properties: { summary: { type: "STRING", description: "A short and clear summary of the action (example: 'Found Flight Ticket to Paris', 'Queried AAPL Stock Price')." }, action_type: { type: "STRING", description: "The type of the action (example: 'flight_search', 'stock_price_query', 'hotel_booking')." }, details: { type: "OBJECT", description: "A JSON object containing all the details of the action. For example, for a flight: { origin, destination, date, price }." } }, required: ["summary", "action_type", "details"] } },
                { name: "find_receipts", description: "Finds and lists the user's past and recorded actions (receipts) for a specific date range.", parameters: { type: "OBJECT", properties: { start_date: { type: "STRING", description: "The start date for the search. Can be in 'YYYY-MM-DD' format or a relative expression like 'today', 'yesterday'." }, end_date: { type: "STRING", description: "The end date for the search. Can be in 'YYYY-MM-DD' format or a relative expression like 'today'." } }, required: ["start_date", "end_date"] } }
            ]
        }
    ],
    zh: [
        {
            functionDeclarations: [
                ...
                { name: "schedule_task", description: "为将来的特定时间安排电子邮件、笔记或文件。如果用户使用像“小时”、“稍后”、“晚上”这样的时间表达，则必须选择此工具。", parameters: { type: "OBJECT", properties: { time: { type: "STRING", description: "任务时间，必须为24小时制的“HH:MM”格式（例如：“17:45”）。" }, noteName: { type: "STRING", description: "要通过电子邮件发送的预存笔记的名称。" }, subject: { type: "STRING", description: "要从头创建的电子邮件的主题。" }, body: { type: "STRING", description: "要从头创建的电子邮件的内容。" }, attachmentDescription: { type: "STRING", description: "要附加到电子邮件的先前保存的记忆（文件）的描述。" } }, required: ["time"] } },
                ...
                { name: "create_calendar_event", description: "将新活动添加到用户的日历中。", parameters: { type: "OBJECT", properties: { title: { type: "STRING" }, date: { type: "STRING", description: "事件日期必须为“YYYY-MM-DD”格式。如果用户说“今天”或“明天”，请自行计算并使用此格式的相应日期。如果未指定日期，则使用今天。" }, time: { type: "STRING", description: "事件时间必须为24小时制的“HH:MM”格式（例如：“14:30”）。" }, description: { type: "STRING" } }, required: ["title", "time"] } },
                ...
                { name: "get_crypto_price", description: "以特定货币获取数字货币（加密货币）的当前价格。", parameters: { type: "OBJECT", properties: { coin_name: { type: "STRING", description: "要获取价格的加密货币的名称（例如：'bitcoin', 'ethereum'）。" }, currency: { type: "STRING", description: "请求价格的货币（例如：'usd', 'eur', 'try'）。" } }, required: ["coin_name", "currency"] } },
                { name: "get_stock_price", description: "获取公司股票的当前市场价格。", parameters: { type: "OBJECT", properties: { stock_symbol: { type: "STRING", description: "要获取价格的股票的市场符号（例如：'AAPL', 'TSLA', 'GOOGL'）。" } }, required: ["stock_symbol"] } },
                { name: "web_search", description: "在互联网上搜索特定主题并查找当前结果。用于机票、比赛门票、酒店价格或一般信息等主题。", parameters: { type: "OBJECT", properties: { query: { type: "STRING", description: "要在互联网上搜索的查询（例如：“伊斯坦布尔最好的意大利餐厅”）。" } }, required: ["query"] } },
                { name: "record_action_receipt", description: "将代表用户成功完成的重要操作（例如，查找航班、查询股价、进行预订等）永久保存为“收据”，以备将来参考。", parameters: { type: "OBJECT", properties: { summary: { type: "STRING", description: "操作的简短明了的摘要（例如：“找到飞往巴黎的机票”，“查询了AAPL股价”）。" }, action_type: { type: "STRING", description: "操作的类型（例如：'flight_search'、'stock_price_query'、'hotel_booking'）。" }, details: { type: "OBJECT", description: "包含操作所有详细信息的JSON对象。例如，对于航班：{ origin, destination, date, price }。" } }, required: ["summary", "action_type", "details"] } },
                { name: "find_receipts", description: "查找并列出用户在特定日期范围内过去进行并已记录的操作（收据）。", parameters: { type: "OBJECT", properties: { start_date: { type: "STRING", description: "搜索的开始日期。可以是“YYYY-MM-DD”格式或相对表达，如“今天”、“昨天”。" }, end_date: { type: "STRING", description: "搜索的结束日期。可以是“YYYY-MM-DD”格式或相对表达，如“今天”。" } }, required: ["start_date", "end_date"] } }
            ]
        }
    ],
    hi: [
        {
            functionDeclarations: [
                ...
                { name: "schedule_task", description: "भविष्य में एक विशिष्ट समय के लिए एक ईमेल, नोट या फ़ाइल शेड्यूल करता है। यदि उपयोगकर्ता 'घंटे', 'बाद में', 'शाम' जैसे समय अभिव्यक्ति का उपयोग करता है तो इस टूल को अनिवार्य रूप से चुना जाना चाहिए।", parameters: { type: "OBJECT", properties: { time: { type: "STRING", description: "कार्य का समय, 24-घंटे के 'HH:MM' प्रारूप में होना चाहिए (उदाहरण: '17:45')।" }, noteName: { type: "STRING", description: "ईमेल के माध्यम से भेजे जाने वाले पूर्व-सहेजे गए नोट का नाम।" }, subject: { type: "STRING", description: "स्क्रैच से बनाए जाने वाले ईमेल का विषय।" }, body: { type: "STRING", description: "स्क्रैच से बनाए जाने वाले ईमेल की सामग्री।" }, attachmentDescription: { type: "STRING", description: "ईमेल में संलग्न की जाने वाली पहले से सहेजी गई मेमोरी (फ़ाइल) का विवरण।" } }, required: ["time"] } },
                ...
                { name: "create_calendar_event", description: "उपयोगकर्ता के कैलेंडर में एक नया ईवेंट या अपॉइंटमेंट जोड़ता है।", parameters: { type: "OBJECT", properties: { title: { type: "STRING" }, date: { type: "STRING", description: "ईवेंट की तारीख 'YYYY-MM-DD' प्रारूप में होनी चाहिए। यदि उपयोगकर्ता 'आज' या 'कल' कहता है, तो इस प्रारूप में संबंधित तारीख की गणना स्वयं करें। यदि कोई तारीख निर्दिष्ट नहीं है, तो आज का उपयोग करें।" }, time: { type: "STRING", description: "ईवेंट का समय 24-घंटे के 'HH:MM' प्रारूप में होना चाहिए (उदाहरण: '14:30')।" }, description: { type: "STRING" } }, required: ["title", "time"] } },
                ...
                { name: "get_crypto_price", description: "एक विशिष्ट मुद्रा में एक डिजिटल मुद्रा (क्रिप्टोकरेंसी) की वर्तमान कीमत प्राप्त करता है।", parameters: { type: "OBJECT", properties: { coin_name: { type: "STRING", description: "कीमत प्राप्त करने के लिए क्रिप्टोकरेंसी का नाम (जैसे: 'bitcoin', 'ethereum')।" }, currency: { type: "STRING", description: "जिस मुद्रा में कीमत का अनुरोध किया गया है (जैसे: 'usd', 'eur', 'try')।" } }, required: ["coin_name", "currency"] } },
                { name: "get_stock_price", description: "एक कंपनी के स्टॉक की वर्तमान शेयर बाजार मूल्य प्राप्त करता है।", parameters: { type: "OBJECT", properties: { stock_symbol: { type: "STRING", description: "कीमत प्राप्त करने के लिए स्टॉक का शेयर बाजार प्रतीक (जैसे: 'AAPL', 'TSLA', 'GOOGL')।" } }, required: ["stock_symbol"] } },
                { name: "web_search", description: "किसी विशिष्ट विषय के बारे में इंटरनेट पर खोज करता है और वर्तमान परिणाम ढूंढता है। इसका उपयोग उड़ान टिकट, मैच टिकट, होटल की कीमतें या सामान्य जानकारी जैसे विषयों के लिए किया जाता है।", parameters: { type: "OBJECT", properties: { query: { type: "STRING", description: "इंटरनेट पर खोजी जाने वाली क्वेरी (उदाहरण: 'इस्तांबुल में सर्वश्रेष्ठ इतालवी रेस्तरां')।" } }, required: ["query"] } },
                { name: "record_action_receipt", description: "उपयोगकर्ता की ओर से सफलतापूर्वक पूरी की गई एक महत्वपूर्ण कार्रवाई (जैसे, उड़ानें ढूंढना, स्टॉक की कीमतों की पूछताछ करना, आरक्षण करना आदि) को भविष्य के संदर्भ के लिए 'रसीद' के रूप में स्थायी रूप से सहेजता है।", parameters: { type: "OBJECT", properties: { summary: { type: "STRING", description: "कार्रवाई का एक संक्षिप्त और स्पष्ट सारांश (उदाहरण: 'पेरिस के लिए उड़ान टिकट मिला', 'AAPL स्टॉक मूल्य की पूछताछ की गई')।" }, action_type: { type: "STRING", description: "कार्रवाई का प्रकार (उदाहरण: 'flight_search', 'stock_price_query', 'hotel_booking')।" }, details: { type: "OBJECT", description: "एक JSON ऑब्जेक्ट जिसमें कार्रवाई के सभी विवरण हों। उदाहरण के लिए, एक उड़ान के लिए: { origin, destination, date, price }।" } }, required: ["summary", "action_type", "details"] } },
                { name: "find_receipts", description: "एक विशिष्ट तिथि सीमा के लिए उपयोगकर्ता के पिछले और रिकॉर्ड किए गए कार्यों (रसीदों) को ढूंढता है।", parameters: { type: "OBJECT", properties: { start_date: { type: "STRING", description: "खोज के लिए प्रारंभ तिथि। 'YYYY-MM-DD' प्रारूप में या 'आज', 'कल' जैसे सापेक्ष अभिव्यक्ति हो सकती है।" }, end_date: { type: "STRING", description: "खोज के लिए समाप्ति तिथि। 'YYYY-MM-DD' प्रारूप में या 'आज' जैसे सापेक्ष अभिव्यक्ति हो सकती है।" } }, required: ["start_date", "end_date"] } }
            ]
        }
    ],
    es: [
        {
            functionDeclarations: [
                ...
                { name: "schedule_task", description: "Programa un correo electrónico, nota o archivo para una hora específica en el FUTURO. Esta herramienta DEBE elegirse si el usuario utiliza una expresión de tiempo como 'hora', 'más tarde', 'noche'.", parameters: { type: "OBJECT", properties: { time: { type: "STRING", description: "La hora de la tarea, DEBE estar en formato de 24 horas 'HH:MM' (ejemplo: '17:45')." }, noteName: { type: "STRING", description: "El nombre de una nota pre-guardada que se enviará por correo electrónico." }, subject: { type: "STRING", description: "El asunto de un correo electrónico que se creará desde cero." }, body: { type: "STRING", description: "El contenido de un correo electrónico que se creará desde cero." }, attachmentDescription: { type: "STRING", description: "La descripción de un recuerdo (archivo) guardado previamente que se adjuntará al correo electrónico." } }, required: ["time"] } },
                ...
                { name: "create_calendar_event", description: "Agrega un nuevo evento o cita al calendario del usuario.", parameters: { type: "OBJECT", properties: { title: { type: "STRING" }, date: { type: "STRING", description: "La fecha del evento DEBE estar en formato 'YYYY-MM-DD'. Si el usuario dice 'hoy' o 'mañana', CALCULA TÚ MISMO la fecha correspondiente en este formato. Si no se especifica ninguna fecha, usa la de hoy." }, time: { type: "STRING", description: "La hora del evento DEBE estar en formato de 24 horas 'HH:MM' (ejemplo: '14:30')." }, description: { type: "STRING" } }, required: ["title", "time"] } },
                ...
                { name: "get_crypto_price", description: "Obtiene el precio actual de una moneda digital (criptomoneda) en una divisa específica.", parameters: { type: "OBJECT", properties: { coin_name: { type: "STRING", description: "El nombre de la criptomoneda de la que se obtendrá el precio (ej: 'bitcoin', 'ethereum')." }, currency: { type: "STRING", description: "La divisa en la que se solicita el precio (ej: 'usd', 'eur', 'try')." } }, required: ["coin_name", "currency"] } },
                { name: "get_stock_price", description: "Obtiene el precio actual en bolsa de las acciones de una empresa.", parameters: { type: "OBJECT", properties: { stock_symbol: { type: "STRING", description: "El símbolo bursátil de la acción de la que se obtendrá el precio (ej: 'AAPL', 'TSLA', 'GOOGL')." } }, required: ["stock_symbol"] } },
                { name: "web_search", description: "Busca en internet sobre un tema específico y encuentra resultados actuales. Se utiliza para temas como billetes de avión, entradas de partidos, precios de hoteles o información general.", parameters: { type: "OBJECT", properties: { query: { type: "STRING", description: "La consulta que se buscará en internet (ejemplo: 'mejores restaurantes italianos en Estambul')." } }, required: ["query"] } },
                { name: "record_action_receipt", description: "Guarda permanentemente una acción importante completada con éxito en nombre del usuario (por ejemplo, encontrar vuelos, consultar precios de acciones, hacer una reserva) como un 'recibo' para referencia futura.", parameters: { type: "OBJECT", properties: { summary: { type: "STRING", description: "Un resumen breve y claro de la acción (ejemplo: 'Billete de avión a París encontrado', 'Precio de la acción de AAPL consultado')." }, action_type: { type: "STRING", description: "El tipo de acción (ejemplo: 'flight_search', 'stock_price_query', 'hotel_booking')." }, details: { type: "OBJECT", description: "Un objeto JSON que contiene todos los detalles de la acción. Por ejemplo, para un vuelo: { origin, destination, date, price }." } }, required: ["summary", "action_type", "details"] } },
                { name: "find_receipts", description: "Encuentra y lista las acciones pasadas y registradas (recibos) del usuario para un rango de fechas específico.", parameters: { type: "OBJECT", properties: { start_date: { type: "STRING", description: "La fecha de inicio de la búsqueda. Puede estar en formato 'YYYY-MM-DD' o ser una expresión relativa como 'hoy', 'ayer'." }, end_date: { type: "STRING", description: "La fecha de finalización de la búsqueda. Puede estar en formato 'YYYY-MM-DD' o ser una expresión relativa como 'hoy'." } }, required: ["start_date", "end_date"] } }
            ]
        }
    ],
    fr: [
        {
            functionDeclarations: [
                ...
                { name: "schedule_task", description: "Planifie un e-mail, une note ou un fichier pour une heure spécifique dans le FUTUR. Cet outil DOIT être choisi si l'utilisateur utilise une expression temporelle comme 'heure', 'plus tard', 'soir'.", parameters: { type: "OBJECT", properties: { time: { type: "STRING", description: "L'heure de la tâche DOIT être au format 24 heures 'HH:MM' (exemple : '17:45')." }, noteName: { type: "STRING", description: "Le nom d'une note pré-enregistrée à envoyer par e-mail." }, subject: { type: "STRING", description: "L'objet d'un e-mail à créer à partir de zéro." }, body: { type: "STRING", description: "Le contenu d'un e-mail à créer à partir de zéro." }, attachmentDescription: { type: "STRING", description: "La description d'un souvenir (fichier) précédemment enregistré à joindre à l'e-mail." } }, required: ["time"] } },
                ...
                { name: "create_calendar_event", description: "Ajoute un nouvel événement ou rendez-vous au calendrier de l'utilisateur.", parameters: { type: "OBJECT", properties: { title: { type: "STRING" }, date: { type: "STRING", description: "La date de l'événement DOIT être au format 'YYYY-MM-DD'. Si l'utilisateur dit 'aujourd'hui' ou 'demain', CALCULEZ VOUS-MÊME la date correspondante dans ce format. Si aucune date n'est spécifiée, utilisez la date d'aujourd'hui." }, time: { type: "STRING", description: "L'heure de l'événement DOIT être au format 24 heures 'HH:MM' (exemple : '14:30')." }, description: { type: "STRING" } }, required: ["title", "time"] } },
                ...
                { name: "get_crypto_price", description: "Obtient le prix actuel d'une monnaie numérique (cryptomonnaie) dans une devise spécifique.", parameters: { type: "OBJECT", properties: { coin_name: { type: "STRING", description: "Le nom de la cryptomonnaie dont on veut obtenir le prix (ex: 'bitcoin', 'ethereum')." }, currency: { type: "STRING", description: "La devise dans laquelle le prix est demandé (ex: 'usd', 'eur', 'try')." } }, required: ["coin_name", "currency"] } },
                { name: "get_stock_price", description: "Obtient le cours actuel de l'action d'une société en bourse.", parameters: { type: "OBJECT", properties: { stock_symbol: { type: "STRING", description: "Le symbole boursier de l'action dont on veut obtenir le prix (ex: 'AAPL', 'TSLA', 'GOOGL')." } }, required: ["stock_symbol"] } },
                { name: "web_search", description: "Effectue une recherche sur Internet sur un sujet spécifique et trouve des résultats actuels. Utilisé pour des sujets tels que les billets d'avion, les billets de match, les prix des hôtels ou des informations générales.", parameters: { type: "OBJECT", properties: { query: { type: "STRING", description: "La requête à rechercher sur Internet (exemple : 'meilleurs restaurants italiens à Istanbul')." } }, required: ["query"] } },
                { name: "record_action_receipt", description: "Enregistre de manière permanente une action importante terminée avec succès au nom de l'utilisateur (par exemple, trouver des vols, consulter des cours d'actions, faire une réservation) en tant que 'reçu' pour référence future.", parameters: { type: "OBJECT", properties: { summary: { type: "STRING", description: "Un résumé court et clair de l'action (exemple : 'Billet d'avion pour Paris trouvé', 'Cours de l'action AAPL consulté')." }, action_type: { type: "STRING", description: "Le type d'action (exemple : 'flight_search', 'stock_price_query', 'hotel_booking')." }, details: { type: "OBJECT", description: "Un objet JSON contenant tous les détails de l'action. Par exemple, pour un vol : { origin, destination, date, price }." } }, required: ["summary", "action_type", "details"] } },
                { name: "find_receipts", description: "Trouve et liste les actions passées et enregistrées (reçus) de l'utilisateur pour une plage de dates spécifique.", parameters: { type: "OBJECT", properties: { start_date: { type: "STRING", description: "La date de début de la recherche. Peut être au format 'YYYY-MM-DD' ou une expression relative comme 'aujourd'hui', 'hier'." }, end_date: { type: "STRING", description: "La date de fin de la recherche. Peut être au format 'YYYY-MM-DD' ou une expression relative comme 'aujourd'hui'." } }, required: ["start_date", "end_date"] } }
            ]
        }
    ],
    ar: [
        {
            functionDeclarations: [
                ...
                { name: "schedule_task", description: "يجدول بريدًا إلكترونيًا أو ملاحظة أو ملفًا لوقت محدد في المستقبل. يجب اختيار هذه الأداة حتمًا إذا استخدم المستخدم تعبيرًا زمنيًا مثل 'ساعة'، 'لاحقًا'، 'مساءً'.", parameters: { type: "OBJECT", properties: { time: { type: "STRING", description: "وقت المهمة، يجب أن يكون بتنسيق 24 ساعة 'HH:MM' (مثال: '17:45')." }, noteName: { type: "STRING", description: "اسم ملاحظة محفوظة مسبقًا ليتم إرسالها عبر البريد الإلكتروني." }, subject: { type: "STRING", description: "موضوع بريد إلكتروني سيتم إنشاؤه من البداية." }, body: { type: "STRING", description: "محتوى بريد إلكتروني سيتم إنشاؤه من البداية." }, attachmentDescription: { type: "STRING", description: "وصف لذاكرة (ملف) محفوظة مسبقًا ليتم إرفاقها بالبريد الإلكتروني." } }, required: ["time"] } },
                ...
                { name: "create_calendar_event", description: "يضيف حدثًا جديدًا أو موعدًا إلى تقويم المستخدم.", parameters: { type: "OBJECT", properties: { title: { type: "STRING" }, date: { type: "STRING", description: "تاريخ الحدث يجب أن يكون بتنسيق 'YYYY-MM-DD'. إذا قال المستخدم 'اليوم' أو 'غدًا'، قم بحساب التاريخ المناظر بهذا التنسيق بنفسك. إذا لم يتم تحديد تاريخ، استخدم تاريخ اليوم." }, time: { type: "STRING", description: "وقت الحدث يجب أن يكون بتنسيق 24 ساعة 'HH:MM' (مثال: '14:30')." }, description: { type: "STRING" } }, required: ["title", "time"] } },
                ...
                { name: "get_crypto_price", description: "الحصول على السعر الحالي لعملة رقمية (عملة مشفرة) بعملة محددة.", parameters: { type: "OBJECT", properties: { coin_name: { type: "STRING", description: "اسم العملة المشفرة للحصول على سعرها (مثال: 'bitcoin', 'ethereum')." }, currency: { type: "STRING", description: "العملة التي يطلب بها السعر (مثال: 'usd', 'eur', 'try')." } }, required: ["coin_name", "currency"] } },
                { name: "get_stock_price", description: "الحصول على سعر سهم شركة حالي في البورصة.", parameters: { type: "OBJECT", properties: { stock_symbol: { type: "STRING", description: "رمز السهم في البورصة للحصول على سعره (مثال: 'AAPL', 'TSLA', 'GOOGL')." } }, required: ["stock_symbol"] } },
                { name: "web_search", description: "يبحث في الإنترنت عن موضوع معين ويجد نتائج حالية. يستخدم لمواضيع مثل تذاكر الطيران، تذاكر المباريات، أسعار الفنادق، أو معلومات عامة.", parameters: { type: "OBJECT", properties: { query: { type: "STRING", description: "الاستعلام الذي سيتم البحث عنه على الإنترنت (مثال: 'أفضل المطاعم الإيطالية في اسطنبول')." } }, required: ["query"] } },
                { name: "record_action_receipt", description: "يحفظ بشكل دائم إجراءً هامًا تم إكماله بنجاح نيابة عن المستخدم (مثل العثور على رحلات جوية، والاستعلام عن أسعار الأسهم، وإجراء حجز) كـ 'إيصال' للرجوع إليه في المستقبل.", parameters: { type: "OBJECT", properties: { summary: { type: "STRING", description: "ملخص قصير وواضح للإجراء (مثال: 'تم العثور على تذكرة طيران إلى باريس'، 'تم الاستعلام عن سعر سهم AAPL')." }, action_type: { type: "STRING", description: "نوع الإجراء (مثال: 'flight_search', 'stock_price_query', 'hotel_booking')." }, details: { type: "OBJECT", description: "كائن JSON يحتوي على جميع تفاصيل الإجراء. على سبيل المثال، لرحلة جوية: { origin, destination, date, price }." } }, required: ["summary", "action_type", "details"] } },
                { name: "find_receipts", description: "يبحث ويسرد الإجراءات السابقة والمسجلة للمستخدم (الإيصالات) لنطاق زمني محدد.", parameters: { type: "OBJECT", properties: { start_date: { type: "STRING", description: "تاريخ بدء البحث. يمكن أن يكون بتنسيق 'YYYY-MM-DD' أو تعبير نسبي مثل 'اليوم'، 'أمس'." }, end_date: { type: "STRING", description: "تاريخ انتهاء البحث. يمكن أن يكون بتنسيق 'YYYY-MM-DD' أو تعبير نسبي مثل 'اليوم'." } }, required: ["start_date", "end_date"] } }
            ]
        }
    ],
    bn: [
        {
            functionDeclarations: [
                ...
                { name: "schedule_task", description: "ভবিষ্যতে একটি নির্দিষ্ট সময়ের জন্য একটি ইমেল, নোট বা ফাইল নির্ধারণ করে। ব্যবহারকারী যদি 'ঘন্টা', 'পরে', 'সন্ধ্যা'র মতো সময় অভিব্যক্তি ব্যবহার করে তবে অবশ্যই এই সরঞ্জামটি বেছে নিতে হবে।", parameters: { type: "OBJECT", properties: { time: { type: "STRING", description: "কাজের সময়টি অবশ্যই 24-ঘন্টার 'HH:MM' বিন্যাসে হতে হবে (উদাহরণস্বরূপ: '17:45')।" }, noteName: { type: "STRING", description: "ইমেলের মাধ্যমে পাঠানোর জন্য একটি পূর্ব-সংরক্ষিত নোটের নাম।" }, subject: { type: "STRING", description: "নতুন করে তৈরি করা একটি ইমেলের বিষয়।" }, body: { type: "STRING", description: "নতুন করে তৈরি করা একটি ইমেলের বিষয়বস্তু।" }, attachmentDescription: { type: "STRING", description: "ইমেলে সংযুক্ত করার জন্য পূর্বে সংরক্ষিত একটি স্মৃতির (ফাইলের) বিবরণ।" } }, required: ["time"] } },
                ...
                { name: "create_calendar_event", description: "ব্যবহারকারীর ক্যালেন্ডারে একটি নতুন ইভেন্ট বা অ্যাপয়েন্টমেন্ট যোগ করে।", parameters: { type: "OBJECT", properties: { title: { type: "STRING" }, date: { type: "STRING", description: "ইভেন্টের তারিখ অবশ্যই 'YYYY-MM-DD' বিন্যাসে হতে হবে। যদি ব্যবহারকারী 'আজ' বা 'কাল' বলে, তাহলে এই বিন্যাসে সংশ্লিষ্ট তারিখটি নিজে গণনা করুন। যদি কোনো তারিখ নির্দিষ্ট না করা হয়, তাহলে আজকের তারিখ ব্যবহার করুন।" }, time: { type: "STRING", description: "ইভেন্টের সময় অবশ্যই 24-ঘন্টার 'HH:MM' বিন্যাসে হতে হবে (উদাহরণস্বরূপ: '14:30')।" }, description: { type: "STRING" } }, required: ["title", "time"] } },
                ...
                { name: "get_crypto_price", description: "একটি নির্দিষ্ট মুদ্রায় একটি ডিজিটাল মুদ্রার (ক্রিপ্টোকারেন্সি) বর্তমান মূল্য প্রাপ্ত করে।", parameters: { type: "OBJECT", properties: { coin_name: { type: "STRING", description: "মূল্য প্রাপ্ত করার জন্য ক্রিপ্টোকারেন্সির নাম (যেমন: 'bitcoin', 'ethereum')।" }, currency: { type: "STRING", description: "যে মুদ্রায় মূল্য অনুরোধ করা হয়েছে (যেমন: 'usd', 'eur', 'try')।" } }, required: ["coin_name", "currency"] } },
                { name: "get_stock_price", description: "একটি কোম্পানির স্টকের বর্তমান শেয়ার বাজারের মূল্য প্রাপ্ত করে।", parameters: { type: "OBJECT", properties: { stock_symbol: { type: "STRING", description: "মূল্য প্রাপ্ত করার জন্য স্টকের শেয়ার বাজারের প্রতীক (যেমন: 'AAPL', 'TSLA', 'GOOGL')।" } }, required: ["stock_symbol"] } },
                { name: "web_search", description: "একটি নির্দিষ্ট বিষয়ে ইন্টারনেটে অনুসন্ধান করে এবং বর্তমান ফলাফল খুঁজে বের করে। এটি ফ্লাইট টিকিট, ম্যাচ টিকিট, হোটেলের দাম বা সাধারণ তথ্যের মতো বিষয়গুলির জন্য ব্যবহৃত হয়।", parameters: { type: "OBJECT", properties: { query: { type: "STRING", description: "ইন্টারনেটে অনুসন্ধান করার জন্য ক্যোয়ারী (উদাহরণ: 'ইস্তাম্বুলের সেরা ইতালিয়ান রেস্তোরাঁ')।" } }, required: ["query"] } },
                { name: "record_action_receipt", description: "ব্যবহারকারীর পক্ষে সফলভাবে সম্পন্ন একটি গুরুত্বপূর্ণ ক্রিয়া (যেমন ফ্লাইট খোঁজা, স্টকের মূল্য জিজ্ঞাসা করা, রিজার্ভেশন করা ইত্যাদি) ভবিষ্যতের রেফারেন্সের জন্য 'রসিদ' হিসাবে স্থায়ীভাবে সংরক্ষণ করে।", parameters: { type: "OBJECT", properties: { summary: { type: "STRING", description: "ক্রিয়াটির একটি সংক্ষিপ্ত এবং স্পষ্ট সারসংক্ষেপ (উদাহরণ: 'প্যারিসের ফ্লাইট টিকিট পাওয়া গেছে', 'AAPL স্টকের মূল্য জিজ্ঞাসা করা হয়েছে')।" }, action_type: { type: "STRING", description: "ক্রিয়াটির ধরণ (উদাহরণ: 'flight_search', 'stock_price_query', 'hotel_booking')।" }, details: { type: "OBJECT", description: "ক্রিয়াটির সমস্ত বিবরণ ধারণকারী একটি JSON অবজেক্ট। উদাহরণস্বরূপ, একটি ফ্লাইটের জন্য: { origin, destination, date, price }।" } }, required: ["summary", "action_type", "details"] } },
                { name: "find_receipts", description: "একটি নির্দিষ্ট তারিখের পরিসরের জন্য ব্যবহারকারীর অতীত এবং রেকর্ড করা ক্রিয়াগুলি (রসিদগুলি) খুঁজে বের করে।", parameters: { type: "OBJECT", properties: { start_date: { type: "STRING", description: "অনুসন্ধানের শুরুর তারিখ। 'YYYY-MM-DD' বিন্যাসে বা 'আজ', 'গতকাল' এর মতো আপেক্ষিক অভিব্যক্তি হতে পারে।" }, end_date: { type: "STRING", description: "অনুসন্ধানের শেষের তারিখ। 'YYYY-MM-DD' বিন্যাসে বা 'আজ' এর মতো আপেক্ষিক অভিব্যক্তি হতে পারে।" } }, required: ["start_date", "end_date"] } }
            ]
        }
    ],
    ru: [
        {
            functionDeclarations: [
                ...
                { name: "schedule_task", description: "Планирует электронное письмо, заметку или файл на определенное время в БУДУЩЕМ. Этот инструмент ОБЯЗАТЕЛЬНО должен быть выбран, если пользователь использует временное выражение, такое как 'час', 'позже', 'вечер'.", parameters: { type: "OBJECT", properties: { time: { type: "STRING", description: "Время задачи, ОБЯЗАТЕЛЬНО должно быть в 24-часовом формате 'HH:MM' (пример: '17:45')." }, noteName: { type: "STRING", description: "Имя предварительно сохраненной заметки для отправки по электронной почте." }, subject: { type: "STRING", description: "Тема электронного письма, создаваемого с нуля." }, body: { type: "STRING", description: "Содержимое электронного письма, создаваемого с нуля." }, attachmentDescription: { type: "STRING", description: "Описание ранее сохраненного воспоминания (файла) для прикрепления к электронному письму." } }, required: ["time"] } },
                ...
                { name: "create_calendar_event", description: "Добавляет новое событие или встречу в календарь пользователя.", parameters: { type: "OBJECT", properties: { title: { type: "STRING" }, date: { type: "STRING", description: "Дата события ОБЯЗАТЕЛЬНО должна быть в формате 'YYYY-MM-DD'. Если пользователь говорит 'сегодня' или 'завтра', ВЫЧИСЛИТЕ соответствующую дату в этом формате САМОСТОЯТЕЛЬНО. Если дата не указана, используйте сегодняшнюю." }, time: { type: "STRING", description: "Время события ОБЯЗАТЕЛЬНО должно быть в 24-часовом формате 'HH:MM' (пример: '14:30')." }, description: { type: "STRING" } }, required: ["title", "time"] } },
                ...
                { name: "get_crypto_price", description: "Получает текущую цену цифровой валюты (криптовалюты) в определенной валюте.", parameters: { type: "OBJECT", properties: { coin_name: { type: "STRING", description: "Название криптовалюты, цену которой нужно получить (например, 'bitcoin', 'ethereum')." }, currency: { type: "STRING", description: "Валюта, в которой запрашивается цена (например, 'usd', 'eur', 'try')." } }, required: ["coin_name", "currency"] } },
                { name: "get_stock_price", description: "Получает текущую рыночную цену акций компании.", parameters: { type: "OBJECT", properties: { stock_symbol: { type: "STRING", description: "Биржевой символ акций, цену которых нужно получить (например, 'AAPL', 'TSLA', 'GOOGL')." } }, required: ["stock_symbol"] } },
                { name: "web_search", description: "Ищет в интернете информацию по определенной теме и находит актуальные результаты. Используется для таких тем, как авиабилеты, билеты на матчи, цены в отелях или общая информация.", parameters: { type: "OBJECT", properties: { query: { type: "STRING", description: "Запрос для поиска в интернете (например, 'лучшие итальянские рестораны в Стамбуле')." } }, required: ["query"] } },
                { name: "record_action_receipt", description: "Постоянно сохраняет важное действие, успешно выполненное от имени пользователя (например, поиск рейсов, запрос цен на акции, бронирование), в виде 'квитанции' для будущего использования.", parameters: { type: "OBJECT", properties: { summary: { type: "STRING", description: "Краткое и понятное описание действия (например, 'Найден билет на самолет в Париж', 'Запрошена цена акций AAPL')." }, action_type: { type: "STRING", description: "Тип действия (например, 'flight_search', 'stock_price_query', 'hotel_booking')." }, details: { type: "OBJECT", description: "Объект JSON, содержащий все детали действия. Например, для рейса: { origin, destination, date, price }." } }, required: ["summary", "action_type", "details"] } },
                { name: "find_receipts", description: "Находит и перечисляет прошлые и записанные действия пользователя (квитанции) за определенный диапазон дат.", parameters: { type: "OBJECT", properties: { start_date: { type: "STRING", description: "Дата начала поиска. Может быть в формате 'YYYY-MM-DD' или относительным выражением, таким как 'сегодня', 'вчера'." }, end_date: { type: "STRING", description: "Дата окончания поиска. Может быть в формате 'YYYY-MM-DD' или относительным выражением, таким как 'сегодня'." } }, required: ["start_date", "end_date"] } }
            ]
        }
    ],
    pt: [
        {
            functionDeclarations: [
                ...
                { name: "schedule_task", description: "Agenda um e-mail, nota ou arquivo para um horário específico no FUTURO. Esta ferramenta DEVE ser escolhida se o usuário usar uma expressão de tempo como 'hora', 'mais tarde', 'noite'.", parameters: { type: "OBJECT", properties: { time: { type: "STRING", description: "A hora da tarefa DEVE estar no formato 24 horas 'HH:MM' (exemplo: '17:45')." }, noteName: { type: "STRING", description: "O nome de uma nota pré-salva a ser enviada por e-mail." }, subject: { type: "STRING", description: "O assunto de um e-mail a ser criado do zero." }, body: { type: "STRING", description: "O conteúdo de um e-mail a ser criado do zero." }, attachmentDescription: { type: "STRING", description: "A descrição de uma memória (arquivo) previamente salva a ser anexada ao e-mail." } }, required: ["time"] } },
                ...
                { name: "create_calendar_event", description: "Adiciona um novo evento ou compromisso ao calendário do usuário.", parameters: { type: "OBJECT", properties: { title: { type: "STRING" }, date: { type: "STRING", description: "A data do evento DEVE estar no formato 'YYYY-MM-DD'. Se o usuário disser 'hoje' ou 'amanhã', CALCULE VOCÊ MESMO a data correspondente neste formato. Se nenhuma data for especificada, use a de hoje." }, time: { type: "STRING", description: "A hora do evento DEVE estar no formato 24 horas 'HH:MM' (exemplo: '14:30')." }, description: { type: "STRING" } }, required: ["title", "time"] } },
                ...
                { name: "get_crypto_price", description: "Obtém o preço atual de uma moeda digital (criptomoeda) em uma moeda específica.", parameters: { type: "OBJECT", properties: { coin_name: { type: "STRING", description: "O nome da criptomoeda para obter o preço (ex: 'bitcoin', 'ethereum')." }, currency: { type: "STRING", description: "A moeda na qual o preço é solicitado (ex: 'usd', 'eur', 'try')." } }, required: ["coin_name", "currency"] } },
                { name: "get_stock_price", description: "Obtém o preço atual de mercado das ações de uma empresa.", parameters: { type: "OBJECT", properties: { stock_symbol: { type: "STRING", description: "O símbolo de mercado da ação para obter o preço (ex: 'AAPL', 'TSLA', 'GOOGL')." } }, required: ["stock_symbol"] } },
                { name: "web_search", description: "Pesquisa na internet por um tópico específico e encontra resultados atuais. Usado para tópicos como passagens aéreas, ingressos para jogos, preços de hotéis ou informações gerais.", parameters: { type: "OBJECT", properties: { query: { type: "STRING", description: "A consulta a ser pesquisada na internet (exemplo: 'melhores restaurantes italianos em Istambul')." } }, required: ["query"] } },
                { name: "record_action_receipt", description: "Salva permanentemente uma ação importante concluída com sucesso em nome do usuário (por exemplo, encontrar voos, consultar preços de ações, fazer uma reserva) como um 'recibo' para referência futura.", parameters: { type: "OBJECT", properties: { summary: { type: "STRING", description: "Um resumo curto e claro da ação (exemplo: 'Passagem aérea para Paris encontrada', 'Preço da ação da AAPL consultado')." }, action_type: { type: "STRING", description: "O tipo de ação (exemplo: 'flight_search', 'stock_price_query', 'hotel_booking')." }, details: { type: "OBJECT", description: "Um objeto JSON contendo todos os detalhes da ação. Por exemplo, para um voo: { origin, destination, date, price }." } }, required: ["summary", "action_type", "details"] } },
                { name: "find_receipts", description: "Encontra e lista as ações passadas e registradas do usuário (recibos) para um intervalo de datas específico.", parameters: { type: "OBJECT", properties: { start_date: { type: "STRING", description: "A data de início da pesquisa. Pode estar no formato 'YYYY-MM-DD' ou ser uma expressão relativa como 'hoje', 'ontem'." }, end_date: { type: "STRING", description: "A data de término da pesquisa. Pode estar no formato 'YYYY-MM-DD' ou ser uma expressão relativa como 'hoje'." } }, required: ["start_date", "end_date"] } }
            ]
        }
    ],
    ur: [
        {
            functionDeclarations: [
                ...
                { name: "send_email", description: "ابھی ایک ای میل بھیجتا ہے۔ میমوری کو اٹیچمنٹ کے طور پر بھیجنے کے لیے 'attachmentDescription' پیرامیٹر استعمال کریں۔", parameters: { type: "OBJECT", properties: { to: { type: "STRING" }, subject: { type: "STRING" }, body: { type: "STRING" }, attachmentDescription: { type: "STRING", description: "ای میل میں اٹیچ کرنے کے لیے پہلے سے محفوظ کردہ میমوری کی تفصیل (مثلاً 'استنبول اور گالاٹا')۔" } }, required: ["to", "subject", "body"] } },
                { name: "schedule_task", description: "مستقبل میں ایک مخصوص وقت کے لیے ای میل، نوٹ، یا فائل کو شیڈول کرتا ہے۔ اگر صارف 'گھنٹہ'، 'بعد میں'، 'شام' جیسے وقت کا اظہار استعمال کرتا ہے تو یہ ٹول لازمی طور پر منتخب کیا جانا چاہیے۔", parameters: { type: "OBJECT", properties: { time: { type: "STRING", description: "کام کا وقت 24 گھنٹے کے 'HH:MM' فارمیٹ میں ہونا چاہیے (مثال: '17:45')۔" }, noteName: { type: "STRING", description: "ای میل کے ذریعے بھیجے جانے والے پہلے سے محفوظ کردہ نوٹ کا نام۔" }, subject: { type: "STRING", description: "شروع سے بنائے جانے والے ای میل کا موضوع۔" }, body: { type: "STRING", description: "شروع سے بنائے جانے والے ای میل کا مواد۔" }, attachmentDescription: { type: "STRING", description: "ای میل میں اٹیچ کی جانے والی پہلے سے محفوظ کردہ میমوری (فائل) کی تفصیل۔" } }, required: ["time"] } },
                ...
                { name: "create_calendar_event", description: "صارف کے کیلنڈر میں نیا ایونٹ یا اپائنٹمنٹ شامل کرتا ہے۔", parameters: { type: "OBJECT", properties: { title: { type: "STRING" }, date: { type: "STRING", description: "ایونٹ کی تاریخ 'YYYY-MM-DD' فارمیٹ میں ہونی چاہیے۔ اگر صارف 'آج' یا 'کل' کہتا ہے تو اس فارمیٹ میں متعلقہ تاریخ کا حساب خود لگائیں۔ اگر کوئی تاریخ متعین نہیں ہے تو آج کا دن استعمال کریں۔" }, time: { type: "STRING", description: "ایونٹ کا وقت 24 گھنٹے کے 'HH:MM' فارمیٹ میں ہونا چاہیے (مثال: '14:30')۔" }, description: { type: "STRING" } }, required: ["title", "time"] } },
                { name: "find_memory", description: "صارف کی طرف سے پہلے محفوظ کردہ میমوری (تصویر، فائل، وغیرہ) کو اس کی تفصیل کی بنیاد پر تلاش کرتا اور ڈھونڈتا ہے۔ اسے '... دکھائیں'، '... ڈھونڈیں'، '... لائیں' جیسے کمانڈز کے لیے استعمال کیا جاتا ہے۔", parameters: { type: "OBJECT", properties: { searchText: { type: "STRING", description: "میموری کو تلاش کرنے کے لیے استعمال ہونے والے کلیدی الفاظ (مثلاً 'لال کار')" } }, required: ["searchText"] } },
                { name: "get_crypto_price", description: "ایک مخصوص کرنسی میں ڈیجیٹل کرنسی (کریپٹو کرنسی) کی موجودہ قیمت حاصل کرتا ہے۔", parameters: { type: "OBJECT", properties: { coin_name: { type: "STRING", description: "قیمت حاصل کرنے کے لیے کریپٹو کرنسی کا نام (مثلاً 'bitcoin', 'ethereum')۔" }, currency: { type: "STRING", description: "جس کرنسی میں قیمت کی درخواست کی گئی ہے (مثلاً 'usd', 'eur', 'try')۔" } }, required: ["coin_name", "currency"] } },
                { name: "get_stock_price", description: "ایک کمپنی کے اسٹاک کی موجودہ اسٹاک مارکیٹ کی قیمت حاصل کرتا ہے۔", parameters: { type: "OBJECT", properties: { stock_symbol: { type: "STRING", description: "قیمت حاصل کرنے کے لیے اسٹاک کا اسٹاک مارکیٹ کا نشان (مثلاً 'AAPL', 'TSLA', 'GOOGL')۔" } }, required: ["stock_symbol"] } },
                { name: "web_search", description: "کسی مخصوص موضوع کے بارے میں انٹرنیٹ پر تلاش کرتا ہے اور موجودہ نتائج تلاش کرتا ہے۔ ہوائی جہاز کے ٹکٹ، میچ کے ٹکٹ، ہوٹل کی قیمتیں، یا عمومی معلومات جیسے موضوعات کے لیے استعمال ہوتا ہے۔", parameters: { type: "OBJECT", properties: { query: { type: "STRING", description: "انٹرنیٹ پر تلاش کی جانے والی استفسار (مثال: 'استنبول میں بہترین اطالوی ریستوراں')۔" } }, required: ["query"] } },
                { name: "record_action_receipt", description: "صارف کی جانب سے کامیابی سے مکمل ہونے والی ایک اہم کارروائی (مثلاً پروازیں تلاش کرنا، اسٹاک کی قیمتیں معلوم کرنا، ریزرویشن کرنا وغیرہ) کو مستقبل کے حوالے کے لیے 'رسید' کے طور پر مستقل طور پر محفوظ کرتا ہے۔", parameters: { type: "OBJECT", properties: { summary: { type: "STRING", description: "کارروائی کا مختصر اور واضح خلاصہ (مثال: 'پیرس کے لیے فلائٹ ٹکٹ مل گیا'، 'AAPL اسٹاک کی قیمت معلوم کی گئی')۔" }, action_type: { type: "STRING", description: "کارروائی کی قسم (مثال: 'flight_search', 'stock_price_query', 'hotel_booking')۔" }, details: { type: "OBJECT", description: "ایک JSON آبجیکٹ جس میں کارروائی کی تمام تفصیلات ہوں۔ مثال کے طور پر، ایک پرواز کے لیے: { origin, destination, date, price }۔" } }, required: ["summary", "action_type", "details"] } },
                { name: "find_receipts", description: "صارف کی ماضی کی اور ریکارڈ شدہ کارروائیوں (رسیدوں) کو ایک مخصوص تاریخ کی حد کے لیے تلاش کرتا ہے۔", parameters: { type: "OBJECT", properties: { start_date: { type: "STRING", description: "تلاش کی başlangıç تاریخ۔ 'YYYY-MM-DD' فارمیٹ میں یا 'آج'، 'کل' جیسے نسبتی اظہار میں ہو سکتا ہے۔" }, end_date: { type: "STRING", description: "تلاش کی اختتامی تاریخ۔ 'YYYY-MM-DD' فارمیٹ میں یا 'آج' جیسے نسبتی اظہار میں ہو سکتا ہے۔" } }, required: ["start_date", "end_date"] } }
            ]
        }
    ],
    ms: [
        {
            functionDeclarations: [
                ...
                { name: "schedule_task", description: "Menjadualkan e-mel, nota, atau fail untuk masa tertentu di MASA DEPAN. Alat ini MESTI dipilih jika pengguna menggunakan ungkapan masa seperti 'jam', 'kemudian', 'petang'.", parameters: { type: "OBJECT", properties: { time: { type: "STRING", description: "Masa tugas, MESTI dalam format 24 jam 'HH:MM' (contoh: '17:45')." }, noteName: { type: "STRING", description: "Nama nota pra-simpan untuk dihantar melalui e-mel." }, subject: { type: "STRING", description: "Subjek e-mel yang akan dibuat dari awal." }, body: { type: "STRING", description: "Kandungan e-mel yang akan dibuat dari awal." }, attachmentDescription: { type: "STRING", description: "Penerangan memori (fail) yang disimpan sebelum ini untuk dilampirkan pada e-mel." } }, required: ["time"] } },
                ...
                { name: "create_calendar_event", description: "Menambah acara atau janji temu baharu pada kalendar pengguna.", parameters: { type: "OBJECT", properties: { title: { type: "STRING" }, date: { type: "STRING", description: "Tarikh acara MESTI dalam format 'YYYY-MM-DD'. Jika pengguna berkata 'hari ini' atau 'esok', KIRA SENDIRI tarikh yang sepadan dalam format ini. Jika tiada tarikh dinyatakan, gunakan hari ini." }, time: { type: "STRING", description: "Masa acara MESTI dalam format 24 jam 'HH:MM' (contoh: '14:30')." }, description: { type: "STRING" } }, required: ["title", "time"] } },
                ...
                { name: "get_crypto_price", description: "Mendapatkan harga semasa mata wang digital (kriptowang) dalam mata wang tertentu.", parameters: { type: "OBJECT", properties: { coin_name: { type: "STRING", description: "Nama kriptowang untuk mendapatkan harga (cth: 'bitcoin', 'ethereum')." }, currency: { type: "STRING", description: "Mata wang di mana harga diminta (cth: 'usd', 'eur', 'try')." } }, required: ["coin_name", "currency"] } },
                { name: "get_stock_price", description: "Mendapatkan harga pasaran saham semasa bagi saham syarikat.", parameters: { type: "OBJECT", properties: { stock_symbol: { type: "STRING", description: "Simbol pasaran saham bagi saham untuk mendapatkan harga (cth: 'AAPL', 'TSLA', 'GOOGL')." } }, required: ["stock_symbol"] } },
                { name: "web_search", description: "Mencari di internet untuk topik tertentu dan mencari hasil semasa. Digunakan untuk topik seperti tiket penerbangan, tiket perlawanan, harga hotel, atau maklumat umum.", parameters: { type: "OBJECT", properties: { query: { type: "STRING", description: "Pertanyaan yang akan dicari di internet (contoh: 'restoran Itali terbaik di Istanbul')." } }, required: ["query"] } },
                { name: "record_action_receipt", description: "Menyimpan secara kekal tindakan penting yang berjaya diselesaikan bagi pihak pengguna (cth., mencari penerbangan, menanyakan harga saham, membuat tempahan) sebagai 'resit' untuk rujukan masa depan.", parameters: { type: "OBJECT", properties: { summary: { type: "STRING", description: "Ringkasan ringkas dan jelas mengenai tindakan tersebut (contoh: 'Tiket Penerbangan ke Paris Ditemui', 'Harga Saham AAPL Ditanya')." }, action_type: { type: "STRING", description: "Jenis tindakan (contoh: 'flight_search', 'stock_price_query', 'hotel_booking')." }, details: { type: "OBJECT", description: "Objek JSON yang mengandungi semua butiran tindakan. Contohnya, untuk penerbangan: { origin, destination, date, price }." } }, required: ["summary", "action_type", "details"] } },
                { name: "find_receipts", description: "Mencari dan menyenaraikan tindakan lalu dan yang direkodkan oleh pengguna (resit) untuk julat tarikh tertentu.", parameters: { type: "OBJECT", properties: { start_date: { type: "STRING", description: "Tarikh mula untuk carian. Boleh dalam format 'YYYY-MM-DD' atau ungkapan relatif seperti 'hari ini', 'semalam'." }, end_date: { type: "STRING", description: "Tarikh tamat untuk carian. Boleh dalam format 'YYYY-MM-DD' atau ungkapan relatif seperti 'hari ini'." } }, required: ["start_date", "end_date"] } }
            ]
        }
    ],
    de: [
        {
            functionDeclarations: [
                ...
                { name: "schedule_task", description: "Plant eine E-Mail, eine Notiz oder eine Datei für eine bestimmte Zeit in der ZUKUNFT. Dieses Werkzeug MUSS ausgewählt werden, wenn der Benutzer einen Zeitausdruck wie 'Stunde', 'später', 'Abend' verwendet.", parameters: { type: "OBJECT", properties: { time: { type: "STRING", description: "Die Zeit der Aufgabe MUSS im 24-Stunden-Format 'HH:MM' sein (Beispiel: '17:45')." }, noteName: { type: "STRING", description: "Der Name einer vorgespeicherten Notiz, die per E-Mail gesendet werden soll." }, subject: { type: "STRING", description: "Der Betreff einer von Grund auf neu zu erstellenden E-Mail." }, body: { type: "STRING", description: "Der Inhalt einer von Grund auf neu zu erstellenden E-Mail." }, attachmentDescription: { type: "STRING", description: "Die Beschreibung einer zuvor gespeicherten Erinnerung (Datei), die an die E-Mail angehängt werden soll." } }, required: ["time"] } },
                ...
                { name: "create_calendar_event", description: "Fügt dem Kalender des Benutzers ein neues Ereignis oder einen neuen Termin hinzu.", parameters: { type: "OBJECT", properties: { title: { type: "STRING" }, date: { type: "STRING", description: "Das Veranstaltungsdatum MUSS im Format 'YYYY-MM-DD' sein. Wenn der Benutzer 'heute' oder 'morgen' sagt, BERECHNEN SIE DAS entsprechende Datum in diesem Format SELBST. Wenn kein Datum angegeben wird, verwenden Sie das heutige Datum." }, time: { type: "STRING", description: "Die Veranstaltungszeit MUSS im 24-Stunden-Format 'HH:MM' sein (Beispiel: '14:30')." }, description: { type: "STRING" } }, required: ["title", "time"] } },
                ...
                { name: "get_crypto_price", description: "Ruft den aktuellen Preis einer digitalen Währung (Kryptowährung) in einer bestimmten Währung ab.", parameters: { type: "OBJECT", properties: { coin_name: { type: "STRING", description: "Der Name der Kryptowährung, deren Preis abgerufen werden soll (z.B. 'bitcoin', 'ethereum')." }, currency: { type: "STRING", description: "Die Währung, in der der Preis angefordert wird (z.B. 'usd', 'eur', 'try')." } }, required: ["coin_name", "currency"] } },
                { name: "get_stock_price", description: "Ruft den aktuellen Börsenkurs der Aktie eines Unternehmens ab.", parameters: { type: "OBJECT", properties: { stock_symbol: { type: "STRING", description: "Das Börsensymbol der Aktie, deren Preis abgerufen werden soll (z.B. 'AAPL', 'TSLA', 'GOOGL')." } }, required: ["stock_symbol"] } },
                { name: "web_search", description: "Sucht im Internet nach einem bestimmten Thema und findet aktuelle Ergebnisse. Wird für Themen wie Flugtickets, Spielkarten, Hotelpreise oder allgemeine Informationen verwendet.", parameters: { type: "OBJECT", properties: { query: { type: "STRING", description: "Die im Internet zu suchende Anfrage (Beispiel: 'beste italienische Restaurants in Istanbul')." } }, required: ["query"] } },
                { name: "record_action_receipt", description: "Speichert eine wichtige, im Namen des Benutzers erfolgreich abgeschlossene Aktion (z. B. Flugsuche, Aktienkursabfrage, Reservierung) dauerhaft als 'Beleg' für zukünftige Referenzen.", parameters: { type: "OBJECT", properties: { summary: { type: "STRING", description: "Eine kurze und klare Zusammenfassung der Aktion (Beispiel: 'Flugticket nach Paris gefunden', 'AAPL-Aktienkurs abgefragt')." }, action_type: { type: "STRING", description: "Die Art der Aktion (Beispiel: 'flight_search', 'stock_price_query', 'hotel_booking')." }, details: { type: "OBJECT", description: "Ein JSON-Objekt, das alle Details der Aktion enthält. Zum Beispiel für einen Flug: { origin, destination, date, price }." } }, required: ["summary", "action_type", "details"] } },
                { name: "find_receipts", description: "Findet und listet die vergangenen und aufgezeichneten Aktionen (Belege) des Benutzers für einen bestimmten Datumsbereich auf.", parameters: { type: "OBJECT", properties: { start_date: { type: "STRING", description: "Das Startdatum für die Suche. Kann im Format 'YYYY-MM-DD' oder ein relativer Ausdruck wie 'heute', 'gestern' sein." }, end_date: { type: "STRING", description: "Das Enddatum für die Suche. Kann im Format 'YYYY-MM-DD' oder ein relativer Ausdruck wie 'heute' sein." } }, required: ["start_date", "end_date"] } }
            ]
        }
    ],
    ja: [
        {
            functionDeclarations: [
                ...
                { name: "schedule_task", description: "Eメール、メモ、またはファイルを将来の特定の時間にスケジュールします。ユーザーが「時間」、「後で」、「夕方」などの時間表現を使用する場合は、このツールを必ず選択する必要があります。", parameters: { type: "OBJECT", properties: { time: { type: "STRING", description: "タスクの時間は、必ず24時間形式の「HH:MM」でなければなりません（例：「17:45」）。" }, noteName: { type: "STRING", description: "Eメールで送信する、事前に保存されたメモの名前。" }, subject: { type: "STRING", description: "最初から作成するEメールの件名。" }, body: { type: "STRING", description: "最初から作成するEメールの内容。" }, attachmentDescription: { type: "STRING", description: "Eメールに添付する、以前に保存した思い出（ファイル）の説明。" } }, required: ["time"] } },
                ...
                { name: "create_calendar_event", description: "ユーザーのカレンダーに新しいイベントや予定を追加します。", parameters: { type: "OBJECT", properties: { title: { type: "STRING" }, date: { type: "STRING", description: "イベントの日付は「YYYY-MM-DD」形式である必要があります。ユーザーが「今日」や「明日」と言った場合は、対応する日付をこの形式で自分で計算してください。日付が指定されていない場合は、今日を使用してください。" }, time: { type: "STRING", description: "イベントの時間は、必ず24時間形式の「HH:MM」でなければなりません（例：「14:30」）。" }, description: { type: "STRING" } }, required: ["title", "time"] } },
                ...
                { name: "get_crypto_price", description: "特定の通貨でのデジタル通貨（暗号通貨）の現在価格を取得します。", parameters: { type: "OBJECT", properties: { coin_name: { type: "STRING", description: "価格を取得する暗号通貨の名前（例：「bitcoin」、「ethereum」）。" }, currency: { type: "STRING", description: "価格を要求する通貨（例：「usd」、「eur」、「try」）。" } }, required: ["coin_name", "currency"] } },
                { name: "get_stock_price", description: "会社の株式の現在の市場価格を取得します。", parameters: { type: "OBJECT", properties: { stock_symbol: { type: "STRING", description: "価格を取得する株式の市場シンボル（例：「AAPL」、「TSLA」、「GOOGL」）。" } }, required: ["stock_symbol"] } },
                { name: "web_search", description: "特定のトピックについてインターネットで検索し、現在の結果を見つけます。航空券、試合のチケット、ホテルの価格、または一般的な情報などのトピックに使用されます。", parameters: { type: "OBJECT", properties: { query: { type: "STRING", description: "インターネットで検索するクエリ（例：「イスタンブールで最高のイタリアンレストラン」）。" } }, required: ["query"] } },
                { name: "record_action_receipt", description: "ユーザーに代わって正常に完了した重要なアクション（例：フライトの検索、株価の照会、予約）を、将来の参照のために「領収書」として恒久的に保存します。", parameters: { type: "OBJECT", properties: { summary: { type: "STRING", description: "アクションの短く明確な要約（例：「パリ行きの航空券が見つかりました」、「AAPLの株価を照会しました」）。" }, action_type: { type: "STRING", description: "アクションの種類（例：「flight_search」、「stock_price_query」、「hotel_booking」）。" }, details: { type: "OBJECT", description: "アクションのすべての詳細を含むJSONオブジェクト。例えば、フライトの場合：{ origin, destination, date, price }。" } }, required: ["summary", "action_type", "details"] } },
                { name: "find_receipts", description: "特定の期間について、ユーザーが過去に行った記録済みのアクション（領収書）を検索します。", parameters: { type: "OBJECT", properties: { start_date: { type: "STRING", description: "検索の開始日。「YYYY-MM-DD」形式または「今日」、「昨日」などの相対的な表現が可能です。" }, end_date: { type: "STRING", description: "検索の終了日。「YYYY-MM-DD」形式または「今日」などの相対的な表現が可能です。" } }, required: ["start_date", "end_date"] } }
            ]
        }
    ],
    fa: [
        {
            functionDeclarations: [
                ...
                { name: "schedule_task", description: "یک ایمیل، یادداشت یا فایل را برای زمان مشخصی در آینده برنامه‌ریزی می‌کند. اگر کاربر از عبارات زمانی مانند 'ساعت'، 'بعداً'، 'عصر' استفاده کند، این ابزار باید حتما انتخاب شود.", parameters: { type: "OBJECT", properties: { time: { type: "STRING", description: "زمان وظیفه، حتماً باید با فرمت 24 ساعته 'HH:MM' باشد (مثال: '17:45')." }, noteName: { type: "STRING", description: "نام یک یادداشت از پیش ذخیره شده برای ارسال از طریق ایمیل." }, subject: { type: "STRING", description: "موضوع ایمیلی که از ابتدا ایجاد می‌شود." }, body: { type: "STRING", description: "محتوای ایمیلی که از ابتدا ایجاد می‌شود." }, attachmentDescription: { type: "STRING", description: "توضیح یک خاطره (فایل) از پیش ذخیره شده برای پیوست به ایمیل." } }, required: ["time"] } },
                ...
                { name: "create_calendar_event", description: "یک رویداد یا قرار ملاقات جدید به تقویم کاربر اضافه می‌کند.", parameters: { type: "OBJECT", properties: { title: { type: "STRING" }, date: { type: "STRING", description: "تاریخ رویداد باید با فرمت 'YYYY-MM-DD' باشد. اگر کاربر عباراتی مانند 'امروز' یا 'فردا' را به کار برد، تاریخ مربوطه را خودتان در این فرمت محاسبه کنید. اگر تاریخی مشخص نشود، از تاریخ امروز استفاده کنید." }, time: { type: "STRING", description: "زمان رویداد حتماً باید با فرمت 24 ساعته 'HH:MM' باشد (مثال: '14:30')." }, description: { type: "STRING" } }, required: ["title", "time"] } },
                ...
                { name: "get_crypto_price", description: "قیمت فعلی یک ارز دیجیتال (ارز رمزنگاری شده) را با یک ارز مشخص دریافت می‌کند.", parameters: { type: "OBJECT", properties: { coin_name: { type: "STRING", description: "نام ارز رمزنگاری شده برای دریافت قیمت (مثال: 'bitcoin', 'ethereum')." }, currency: { type: "STRING", description: "ارزی که قیمت با آن درخواست می‌شود (مثال: 'usd', 'eur', 'try')." } }, required: ["coin_name", "currency"] } },
                { name: "get_stock_price", description: "قیمت فعلی سهام یک شرکت در بورس را دریافت می‌کند.", parameters: { type: "OBJECT", properties: { stock_symbol: { type: "STRING", description: "نماد بورسی سهام برای دریافت قیمت (مثال: 'AAPL', 'TSLA', 'GOOGL')." } }, required: ["stock_symbol"] } },
                { name: "web_search", description: "در مورد یک موضوع خاص در اینترنت جستجو می‌کند و نتایج فعلی را پیدا می‌کند. برای موضوعاتی مانند بلیط هواپیما، بلیط مسابقه، قیمت هتل یا اطلاعات عمومی استفاده می‌شود.", parameters: { type: "OBJECT", properties: { query: { type: "STRING", description: "عبارتی که در اینترنت جستجو می‌شود (مثال: 'بهترین رستوران‌های ایتالیایی در استانبول')." } }, required: ["query"] } },
                { name: "record_action_receipt", description: "یک اقدام مهم که با موفقیت به نمایندگی از کاربر انجام شده (مثلاً پیدا کردن پرواز، استعلام قیمت سهام، رزرو کردن) را به عنوان یک 'رسید' برای مراجعه در آینده به طور دائم ذخیره می‌کند.", parameters: { type: "OBJECT", properties: { summary: { type: "STRING", description: "خلاصه کوتاه و واضح از اقدام (مثال: 'بلیط پرواز به پاریس پیدا شد'، 'قیمت سهام AAPL استعلام شد')." }, action_type: { type: "STRING", description: "نوع اقدام (مثال: 'flight_search', 'stock_price_query', 'hotel_booking')." }, details: { type: "OBJECT", description: "یک شیء JSON حاوی تمام جزئیات اقدام. به عنوان مثال، برای یک پرواز: { origin, destination, date, price }." } }, required: ["summary", "action_type", "details"] } },
                { name: "find_receipts", description: "اقدامات گذشته و ثبت شده کاربر (رسیدها) را برای یک بازه زمانی مشخص پیدا می‌کند.", parameters: { type: "OBJECT", properties: { start_date: { type: "STRING", description: "تاریخ شروع جستجو. می‌تواند با فرمت 'YYYY-MM-DD' یا یک عبارت نسبی مانند 'امروز'، 'دیروز' باشد." }, end_date: { type: "STRING", description: "تاریخ پایان جستجو. می‌تواند با فرمت 'YYYY-MM-DD' یا یک عبارت نسبی مانند 'امروز' باشد." } }, required: ["start_date", "end_date"] } }
            ]
        }
    ],
    ha: [
        {
            functionDeclarations: [
                ...
                { name: "schedule_task", description: "Shirya imel, rubutu, ko fayil don takamaiman lokaci a GABA. DOLE ne a zaɓi wannan kayan aiki idan mai amfani ya yi amfani da furucin lokaci kamar 'sa'a', 'daga baya', 'maraice'.", parameters: { type: "OBJECT", properties: { time: { type: "STRING", description: "Lokacin aikin, DOLE ne ya kasance a cikin tsarin 'HH:MM' na sa'o'i 24 (misali: '17:45')." }, noteName: { type: "STRING", description: "Sunan rubutun da aka riga aka adana don a aika ta imel." }, subject: { type: "STRING", description: "Taken imel da za a ƙirƙira daga farko." }, body: { type: "STRING", description: "Abun cikin imel da za a ƙirƙira daga farko." }, attachmentDescription: { type: "STRING", description: "Bayanin ƙwaƙwalwar (fayil) da aka riga aka adana don a haɗa shi da imel." } }, required: ["time"] } },
                ...
                { name: "create_calendar_event", description: "Ƙara sabon taro ko alƙawari a kalandar mai amfani.", parameters: { type: "OBJECT", properties: { title: { type: "STRING" }, date: { type: "STRING", description: "Kwanan watan taron DOLE ne ya kasance a tsarin 'YYYY-MM-DD'. Idan mai amfani ya ce 'yau' ko 'gobe', LISSAFA kwanan watan da ya dace a wannan tsarin DA KANKA. Idan ba a fayyace kwanan wata ba, yi amfani da na yau." }, time: { type: "STRING", description: "Lokacin taron DOLE ne ya kasance a cikin tsarin 'HH:MM' na sa'o'i 24 (misali: '14:30')." }, description: { type: "STRING" } }, required: ["title", "time"] } },
                ...
                { name: "get_crypto_price", description: "Samun farashin yanzu na kuɗin dijital (cryptocurrency) a cikin takamaiman kuɗi.", parameters: { type: "OBJECT", properties: { coin_name: { type: "STRING", description: "Sunan cryptocurrency don samun farashin (misali: 'bitcoin', 'ethereum')." }, currency: { type: "STRING", description: "Kuɗin da aka nema farashin a ciki (misali: 'usd', 'eur', 'try')." } }, required: ["coin_name", "currency"] } },
                { name: "get_stock_price", description: "Samun farashin kasuwar yanzu na hannun jarin kamfani.", parameters: { type: "OBJECT", properties: { stock_symbol: { type: "STRING", description: "Alamar kasuwar hannun jari na hannun jarin don samun farashin (misali: 'AAPL', 'TSLA', 'GOOGL')." } }, required: ["stock_symbol"] } },
                { name: "web_search", description: "Bincika intanet don takamaiman batu kuma nemo sakamako na yanzu. Ana amfani da shi don batutuwa kamar tikitin jirgi, tikitin wasa, farashin otal, ko bayanai na gaba ɗaya.", parameters: { type: "OBJECT", properties: { query: { type: "STRING", description: "Tambayar da za a bincika a intanet (misali: 'mafi kyawun gidajen cin abinci na Italiya a Istanbul')." } }, required: ["query"] } },
                { name: "record_action_receipt", description: "Adana wani muhimmin aiki da aka kammala cikin nasara a madadin mai amfani (misali, nemo jirage, tambayar farashin hannun jari, yin ajiyar wuri) a matsayin 'rasit' don amfanin gaba.", parameters: { type: "OBJECT", properties: { summary: { type: "STRING", description: "Takaitaccen bayani mai sauƙin fahimta game da aikin (misali: 'An samo Tikitin Jirgi zuwa Paris', 'An tambayi Farashin Hannun Jari na AAPL')." }, action_type: { type: "STRING", description: "Nau'in aikin (misali: 'flight_search', 'stock_price_query', 'hotel_booking')." }, details: { type: "OBJECT", description: "Wani abu na JSON wanda ke ƙunshe da dukkan bayanan aikin. Misali, don jirgi: { asali, makoma, kwanan wata, farashi }." } }, required: ["summary", "action_type", "details"] } },
                { name: "find_receipts", description: "Nemo ayyukan da suka gabata da aka adana na mai amfani (rasit) don takamaiman kewayon kwanan wata.", parameters: { type: "OBJECT", properties: { start_date: { type: "STRING", description: "Ranar fara bincike. Zai iya kasancewa a tsarin 'YYYY-MM-DD' ko kuma furuci mai alaƙa kamar 'yau', 'jiya'." }, end_date: { type: "STRING", description: "Ranar ƙare bincike. Zai iya kasancewa a tsarin 'YYYY-MM-DD' ko kuma furuci mai alaƙa kamar 'yau'." } }, required: ["start_date", "end_date"] } }
            ]
        }
    ],
    sw: [
        {
            functionDeclarations: [
                ...
                { name: "schedule_task", description: "Hupanga barua pepe, dokezo, au faili kwa wakati maalum ujao. Zana hii LAZIMA ichaguliwe ikiwa mtumiaji atatumia msemo wa wakati kama 'saa', 'baadaye', 'jioni'.", parameters: { type: "OBJECT", properties: { time: { type: "STRING", description: "Saa ya kazi, LAZIMA iwe katika umbizo la saa 24 'HH:MM' (mfano: '17:45')." }, noteName: { type: "STRING", description: "Jina la dokezo lililohifadhiwa awali la kutumwa kupitia barua pepe." }, subject: { type: "STRING", description: "Mada ya barua pepe itakayoundwa kutoka mwanzo." }, body: { type: "STRING", description: "Maudhui ya barua pepe yatakayoundwa kutoka mwanzo." }, attachmentDescription: { type: "STRING", description: "Maelezo ya kumbukumbu (faili) iliyohifadhiwa awali ya kuambatisha kwenye barua pepe." } }, required: ["time"] } },
                ...
                { name: "create_calendar_event", description: "Huongeza tukio jipya au miadi kwenye kalenda ya mtumiaji.", parameters: { type: "OBJECT", properties: { title: { type: "STRING" }, date: { type: "STRING", description: "Tarehe ya tukio LAZIMA iwe katika umbizo la 'YYYY-MM-DD'. Ikiwa mtumiaji atasema 'leo' au 'kesho', KOKOTOA tarehe husika katika umbizo hili WEWE MWENYEWE. Ikiwa hakuna tarehe iliyobainishwa, tumia ya leo." }, time: { type: "STRING", description: "Saa ya tukio LAZIMA iwe katika umbizo la saa 24 'HH:MM' (mfano: '14:30')." }, description: { type: "STRING" } }, required: ["title", "time"] } },
                ...
                { name: "get_crypto_price", description: "Hupata bei ya sasa ya sarafu ya kidijitali (cryptocurrency) katika sarafu maalum.", parameters: { type: "OBJECT", properties: { coin_name: { type: "STRING", description: "Jina la cryptocurrency ili kupata bei (k.m., 'bitcoin', 'ethereum')." }, currency: { type: "STRING", description: "Sarafu ambayo bei inaombwa (k.m., 'usd', 'eur', 'try')." } }, required: ["coin_name", "currency"] } },
                { name: "get_stock_price", description: "Hupata bei ya sasa ya soko ya hisa ya kampuni.", parameters: { type: "OBJECT", properties: { stock_symbol: { type: "STRING", description: "Alama ya soko ya hisa ya hisa ili kupata bei (k.m., 'AAPL', 'TSLA', 'GOOGL')." } }, required: ["stock_symbol"] } },
                { name: "web_search", description: "Hutafuta kwenye mtandao mada maalum na kupata matokeo ya sasa. Hutumika kwa mada kama tikiti za ndege, tikiti za mechi, bei za hoteli, au taarifa za jumla.", parameters: { type: "OBJECT", properties: { query: { type: "STRING", description: "Hoja itakayotafutwa kwenye mtandao (mfano: 'migahawa bora ya Kiitaliano jijini Istanbul')." } }, required: ["query"] } },
                { name: "record_action_receipt", description: "Huhifadhi kabisa kitendo muhimu kilichokamilishwa kwa mafanikio kwa niaba ya mtumiaji (k.m., kutafuta safari za ndege, kuuliza bei za hisa, kuweka nafasi) kama 'risiti' kwa marejeleo ya baadaye.", parameters: { type: "OBJECT", properties: { summary: { type: "STRING", description: "Muhtasari mfupi na wazi wa kitendo (mfano: 'Tikiti ya Ndege ya kwenda Paris imepatikana', 'Bei ya Hisa ya AAPL imeulizwa')." }, action_type: { type: "STRING", description: "Aina ya kitendo (mfano: 'flight_search', 'stock_price_query', 'hotel_booking')." }, details: { type: "OBJECT", description: "Kitu cha JSON chenye maelezo yote ya kitendo. Kwa mfano, kwa safari ya ndege: { origin, destination, date, price }." } }, required: ["summary", "action_type", "details"] } },
                { name: "find_receipts", description: "Hupata vitendo vya zamani na vilivyorekodiwa vya mtumiaji (risiti) kwa kipindi maalum cha tarehe.", parameters: { type: "OBJECT", properties: { start_date: { type: "STRING", description: "Tarehe ya kuanza kwa utafutaji. Inaweza kuwa katika umbizo la 'YYYY-MM-DD' au msemo linganishi kama 'leo', 'jana'." }, end_date: { type: "STRING", description: "Tarehe ya kumalizika kwa utafutaji. Inaweza kuwa katika umbizo la 'YYYY-MM-DD' au msemo linganishi kama 'leo'." } }, required: ["start_date", "end_date"] } }
            ]
        }
    ],
    vi: [
        {
            functionDeclarations: [
                ...
                { name: "schedule_task", description: "Lên lịch cho email, ghi chú hoặc tệp vào một thời điểm cụ thể trong TƯƠNG LAI. Công cụ này PHẢI được chọn nếu người dùng sử dụng các biểu thức thời gian như 'giờ', 'sau', 'buổi tối'.", parameters: { type: "OBJECT", properties: { time: { type: "STRING", description: "Thời gian của nhiệm vụ, PHẢI ở định dạng 24 giờ 'HH:MM' (ví dụ: '17:45')." }, noteName: { type: "STRING", description: "Tên của một ghi chú đã được lưu trước đó sẽ được gửi qua email." }, subject: { type: "STRING", description: "Chủ đề của một email sẽ được tạo từ đầu." }, body: { type: "STRING", description: "Nội dung của một email sẽ được tạo từ đầu." }, attachmentDescription: { type: "STRING", description: "Mô tả về một kỷ niệm (tệp) đã được lưu trước đó để đính kèm vào email." } }, required: ["time"] } },
                ...
                { name: "create_calendar_event", description: "Thêm một sự kiện hoặc cuộc hẹn mới vào lịch của người dùng.", parameters: { type: "OBJECT", properties: { title: { type: "STRING" }, date: { type: "STRING", description: "Ngày sự kiện PHẢI ở định dạng 'YYYY-MM-DD'. Nếu người dùng nói 'hôm nay' hoặc 'ngày mai', hãy TỰ TÍNH ngày tương ứng ở định dạng này. Nếu không có ngày nào được chỉ định, hãy sử dụng ngày hôm nay." }, time: { type: "STRING", description: "Thời gian sự kiện PHẢI ở định dạng 24 giờ 'HH:MM' (ví dụ: '14:30')." }, description: { type: "STRING" } }, required: ["title", "time"] } },
                ...
                { name: "get_crypto_price", description: "Lấy giá hiện tại của một loại tiền kỹ thuật số (tiền điện tử) bằng một loại tiền tệ cụ thể.", parameters: { type: "OBJECT", properties: { coin_name: { type: "STRING", description: "Tên của loại tiền điện tử để lấy giá (ví dụ: 'bitcoin', 'ethereum')." }, currency: { type: "STRING", description: "Loại tiền tệ mà giá được yêu cầu (ví dụ: 'usd', 'eur', 'try')." } }, required: ["coin_name", "currency"] } },
                { name: "get_stock_price", description: "Lấy giá thị trường chứng khoán hiện tại của cổ phiếu của một công ty.", parameters: { type: "OBJECT", properties: { stock_symbol: { type: "STRING", description: "Ký hiệu thị trường chứng khoán của cổ phiếu để lấy giá (ví dụ: 'AAPL', 'TSLA', 'GOOGL')." } }, required: ["stock_symbol"] } },
                { name: "web_search", description: "Tìm kiếm trên internet về một chủ đề cụ thể và tìm kết quả hiện tại. Được sử dụng cho các chủ đề như vé máy bay, vé trận đấu, giá khách sạn hoặc thông tin chung.", parameters: { type: "OBJECT", properties: { query: { type: "STRING", description: "Truy vấn sẽ được tìm kiếm trên internet (ví dụ: 'nhà hàng Ý tốt nhất ở Istanbul')." } }, required: ["query"] } },
                { name: "record_action_receipt", description: "Lưu vĩnh viễn một hành động quan trọng đã hoàn thành thành công thay mặt người dùng (ví dụ: tìm chuyến bay, truy vấn giá cổ phiếu, đặt chỗ) dưới dạng 'biên lai' để tham khảo trong tương lai.", parameters: { type: "OBJECT", properties: { summary: { type: "STRING", description: "Tóm tắt ngắn gọn và rõ ràng về hành động (ví dụ: 'Đã tìm thấy vé máy bay đến Paris', 'Đã truy vấn giá cổ phiếu AAPL')." }, action_type: { type: "STRING", description: "Loại hành động (ví dụ: 'flight_search', 'stock_price_query', 'hotel_booking')." }, details: { type: "OBJECT", description: "Một đối tượng JSON chứa tất cả các chi tiết của hành động. Ví dụ, đối với một chuyến bay: { origin, destination, date, price }." } }, required: ["summary", "action_type", "details"] } },
                { name: "find_receipts", description: "Tìm và liệt kê các hành động trong quá khứ và đã được ghi lại của người dùng (biên lai) trong một phạm vi ngày cụ thể.", parameters: { type: "OBJECT", properties: { start_date: { type: "STRING", description: "Ngày bắt đầu tìm kiếm. Có thể ở định dạng 'YYYY-MM-DD' hoặc một biểu thức tương đối như 'hôm nay', 'hôm qua'." }, end_date: { type: "STRING", description: "Ngày kết thúc tìm kiếm. Có thể ở định dạng 'YYYY-MM-DD' hoặc một biểu thức tương đối như 'hôm nay'." } }, required: ["start_date", "end_date"] } }
            ]
        }
    ],
    ko: [
        {
            functionDeclarations: [
                ...
                { name: "schedule_task", description: "이메일, 메모 또는 파일을 미래의 특정 시간으로 예약합니다. 사용자가 '시간', '나중에', '저녁'과 같은 시간 표현을 사용하면 반드시 이 도구를 선택해야 합니다.", parameters: { type: "OBJECT", properties: { time: { type: "STRING", description: "작업 시간은 반드시 24시간 형식인 'HH:MM'이어야 합니다(예: '17:45')." }, noteName: { type: "STRING", description: "이메일로 보낼 미리 저장된 메모의 이름." }, subject: { type: "STRING", description: "처음부터 만들 이메일의 제목." }, body: { type: "STRING", description: "처음부터 만들 이메일의 내용." }, attachmentDescription: { type: "STRING", description: "이메일에 첨부할 이전에 저장된 추억(파일)에 대한 설명." } }, required: ["time"] } },
                ...
                { name: "create_calendar_event", description: "사용자의 캘린더에 새 이벤트나 약속을 추가합니다.", parameters: { type: "OBJECT", properties: { title: { type: "STRING" }, date: { type: "STRING", description: "이벤트 날짜는 'YYYY-MM-DD' 형식이어야 합니다. 사용자가 '오늘'이나 '내일'이라고 말하면 해당 날짜를 이 형식으로 직접 계산하십시오. 날짜가 지정되지 않은 경우 오늘을 사용하십시오." }, time: { type: "STRING", description: "이벤트 시간은 반드시 24시간 형식인 'HH:MM'이어야 합니다(예: '14:30')." }, description: { type: "STRING" } }, required: ["title", "time"] } },
                ...
                { name: "get_crypto_price", description: "특정 통화로 디지털 통화(암호화폐)의 현재 가격을 가져옵니다.", parameters: { type: "OBJECT", properties: { coin_name: { type: "STRING", description: "가격을 가져올 암호화폐의 이름 (예: 'bitcoin', 'ethereum')." }, currency: { type: "STRING", description: "가격이 요청된 통화 (예: 'usd', 'eur', 'try')." } }, required: ["coin_name", "currency"] } },
                { name: "get_stock_price", description: "회사의 주식 현재 주가를 가져옵니다.", parameters: { type: "OBJECT", properties: { stock_symbol: { type: "STRING", description: "가격을 가져올 주식의 주식 시장 기호 (예: 'AAPL', 'TSLA', 'GOOGL')." } }, required: ["stock_symbol"] } },
                { name: "web_search", description: "특정 주제에 대해 인터넷을 검색하고 현재 결과를 찾습니다. 항공권, 경기 티켓, 호텔 가격 또는 일반 정보와 같은 주제에 사용됩니다.", parameters: { type: "OBJECT", properties: { query: { type: "STRING", description: "인터넷에서 검색할 쿼리 (예: '이스탄불 최고의 이탈리아 레스토랑')." } }, required: ["query"] } },
                { name: "record_action_receipt", description: "사용자를 대신하여 성공적으로 완료된 중요한 작업(예: 항공편 찾기, 주가 조회, 예약하기 등)을 나중에 참조할 수 있도록 '영수증'으로 영구적으로 저장합니다.", parameters: { type: "OBJECT", properties: { summary: { type: "STRING", description: "작업에 대한 짧고 명확한 요약 (예: '파리행 항공권 찾음', 'AAPL 주가 조회됨')." }, action_type: { type: "STRING", description: "작업 유형 (예: 'flight_search', 'stock_price_query', 'hotel_booking')." }, details: { type: "OBJECT", description: "작업의 모든 세부 정보가 포함된 JSON 객체. 예: 항공편의 경우: { origin, destination, date, price }." } }, required: ["summary", "action_type", "details"] } },
                { name: "find_receipts", description: "특정 날짜 범위에 대해 사용자의 과거 및 기록된 작업(영수증)을 찾아 목록을 표시합니다.", parameters: { type: "OBJECT", properties: { start_date: { type: "STRING", description: "검색 시작 날짜. 'YYYY-MM-DD' 형식이거나 '오늘', '어제'와 같은 상대적인 표현일 수 있습니다." }, end_date: { type: "STRING", description: "검색 종료 날짜. 'YYYY-MM-DD' 형식이거나 '오늘'과 같은 상대적인 표현일 수 있습니다." } }, required: ["start_date", "end_date"] } }
            ]
        }
    ],
    it: [
        {
            functionDeclarations: [
                ...
                { name: "schedule_task", description: "Pianifica un'e-mail, una nota o un file per un'ora specifica nel FUTURO. Questo strumento DEVE essere scelto se l'utente utilizza un'espressione di tempo come 'ora', 'più tardi', 'sera'.", parameters: { type: "OBJECT", properties: { time: { type: "STRING", description: "L'ora dell'attività DEVE essere in formato 24 ore 'HH:MM' (esempio: '17:45')." }, noteName: { type: "STRING", description: "Il nome di una nota pre-salvata da inviare via e-mail." }, subject: { type: "STRING", description: "L'oggetto di un'e-mail da creare da zero." }, body: { type: "STRING", description: "Il contenuto di un'e-mail da creare da zero." }, attachmentDescription: { type: "STRING", description: "La descrizione di un ricordo (file) salvato in precedenza da allegare all'e-mail." } }, required: ["time"] } },
                ...
                { name: "create_calendar_event", description: "Aggiunge un nuovo evento o appuntamento al calendario dell'utente.", parameters: { type: "OBJECT", properties: { title: { type: "STRING" }, date: { type: "STRING", description: "La data dell'evento DEVE essere in formato 'YYYY-MM-DD'. Se l'utente dice 'oggi' o 'domani', CALCOLA TU STESSO la data corrispondente in questo formato. Se non viene specificata alcuna data, usa quella di oggi." }, time: { type: "STRING", description: "L'ora dell'evento DEVE essere in formato 24 ore 'HH:MM' (esempio: '14:30')." }, description: { type: "STRING" } }, required: ["title", "time"] } },
                ...
                { name: "get_crypto_price", description: "Ottiene il prezzo attuale di una valuta digitale (criptovaluta) in una valuta specifica.", parameters: { type: "OBJECT", properties: { coin_name: { type: "STRING", description: "Il nome della criptovaluta di cui ottenere il prezzo (es. 'bitcoin', 'ethereum')." }, currency: { type: "STRING", description: "La valuta in cui viene richiesto il prezzo (es. 'usd', 'eur', 'try')." } }, required: ["coin_name", "currency"] } },
                { name: "get_stock_price", description: "Ottiene il prezzo di mercato attuale delle azioni di una società.", parameters: { type: "OBJECT", properties: { stock_symbol: { type: "STRING", description: "Il simbolo di borsa dell'azione di cui ottenere il prezzo (es. 'AAPL', 'TSLA', 'GOOGL')." } }, required: ["stock_symbol"] } },
                { name: "web_search", description: "Cerca in internet un argomento specifico e trova risultati aggiornati. Utilizzato per argomenti come biglietti aerei, biglietti per partite, prezzi di hotel o informazioni generali.", parameters: { type: "OBJECT", properties: { query: { type: "STRING", description: "La query da cercare su internet (esempio: 'migliori ristoranti italiani a Istanbul')." } }, required: ["query"] } },
                { name: "record_action_receipt", description: "Salva in modo permanente un'azione importante completata con successo per conto dell'utente (ad es. ricerca di voli, richiesta di prezzi di azioni, prenotazione) come 'ricevuta' per riferimenti futuri.", parameters: { type: "OBJECT", properties: { summary: { type: "STRING", description: "Un riepilogo breve e chiaro dell'azione (esempio: 'Biglietto aereo per Parigi trovato', 'Prezzo delle azioni AAPL richiesto')." }, action_type: { type: "STRING", description: "Il tipo di azione (esempio: 'flight_search', 'stock_price_query', 'hotel_booking')." }, details: { type: "OBJECT", description: "Un oggetto JSON contenente tutti i dettagli dell'azione. Ad esempio, per un volo: { origin, destination, date, price }." } }, required: ["summary", "action_type", "details"] } },
                { name: "find_receipts", description: "Trova ed elenca le azioni passate e registrate dell'utente (ricevute) per un intervallo di date specifico.", parameters: { type: "OBJECT", properties: { start_date: { type: "STRING", description: "La data di inizio della ricerca. Può essere in formato 'YYYY-MM-DD' o un'espressione relativa come 'oggi', 'ieri'." }, end_date: { type: "STRING", description: "La data di fine della ricerca. Può essere in formato 'YYYY-MM-DD' o un'espressione relativa come 'oggi'." } }, required: ["start_date", "end_date"] } }
            ]
        }
    ],
    jv: [
        {
            functionDeclarations: [
                ...
                { name: "schedule_task", description: "Njadwalake email, cathetan, utawa file kanggo wektu tartamtu ing MASA DEPAN. Alat iki KUDU dipilih yen pangguna nggunakake ekspresi wektu kaya 'jam', 'mengko', 'sore'.", parameters: { type: "OBJECT", properties: { time: { type: "STRING", description: "Wektu tugas, KUDU ing format 24 jam 'HH:MM' (contone: '17:45')." }, noteName: { type: "STRING", description: "Jeneng cathetan sing wis disimpen kanggo dikirim liwat email." }, subject: { type: "STRING", description: "Subyek email sing bakal digawe saka awal." }, body: { type: "STRING", description: "Isi email sing bakal digawe saka awal." }, attachmentDescription: { type: "STRING", description: "Deskripsi memori (file) sing wis disimpen kanggo dilampirake ing email." } }, required: ["time"] } },
                ...
                { name: "create_calendar_event", description: "Nambahake acara utawa janjian anyar ing tanggalan pangguna.", parameters: { type: "OBJECT", properties: { title: { type: "STRING" }, date: { type: "STRING", description: "Tanggal acara KUDU ing format 'YYYY-MM-DD'. Yen pangguna ngomong 'dina iki' utawa 'sesuk', ITUNGEN DHEWE tanggal sing cocog ing format iki. Yen ora ana tanggal sing ditemtokake, gunakake dina iki." }, time: { type: "STRING", description: "Wektu acara KUDU ing format 24 jam 'HH:MM' (contone: '14:30')." }, description: { type: "STRING" } }, required: ["title", "time"] } },
                ...
                { name: "get_crypto_price", description: "Njaluk rega saiki saka mata uang digital (cryptocurrency) ing mata uang tartamtu.", parameters: { type: "OBJECT", properties: { coin_name: { type: "STRING", description: "Jeneng cryptocurrency kanggo njaluk rega (contone: 'bitcoin', 'ethereum')." }, currency: { type: "STRING", description: "Mata uang sing regane dijaluk (contone: 'usd', 'eur', 'try')." } }, required: ["coin_name", "currency"] } },
                { name: "get_stock_price", description: "Njaluk rega pasar saham saiki saka saham perusahaan.", parameters: { type: "OBJECT", properties: { stock_symbol: { type: "STRING", description: "Simbol pasar saham saka saham kanggo njaluk rega (contone: 'AAPL', 'TSLA', 'GOOGL')." } }, required: ["stock_symbol"] } },
                { name: "web_search", description: "Nggoleki internet kanggo topik tartamtu lan nemokake asil saiki. Digunakake kanggo topik kaya tiket pesawat, tiket pertandingan, rega hotel, utawa informasi umum.", parameters: { type: "OBJECT", properties: { query: { type: "STRING", description: "Pitakonan sing bakal digoleki ing internet (contone: 'restoran Italia paling apik ing Istanbul')." } }, required: ["query"] } },
                { name: "record_action_receipt", description: "Nyimpen kanthi permanen tindakan penting sing kasil dirampungake atas jenenge pangguna (contone, nemokake penerbangan, takon rega saham, nggawe reservasi) minangka 'kuitansi' kanggo referensi mbesuk.", parameters: { type: "OBJECT", properties: { summary: { type: "STRING", description: "Ringkesan singkat lan jelas saka tindakan (contone: 'Tiket Pesawat menyang Paris Ditemokake', 'Rega Saham AAPL Ditakokake')." }, action_type: { type: "STRING", description: "Jinis tindakan (contone: 'flight_search', 'stock_price_query', 'hotel_booking')." }, details: { type: "OBJECT", description: "Objek JSON sing ngemot kabeh rincian tindakan. Contone, kanggo penerbangan: { origin, destination, date, price }." } }, required: ["summary", "action_type", "details"] } },
                { name: "find_receipts", description: "Nemokake lan ndhaptar tindakan pangguna sing kepungkur lan sing wis direkam (kuitansi) kanggo rentang tanggal tartamtu.", parameters: { type: "OBJECT", properties: { start_date: { type: "STRING", description: "Tanggal wiwitan kanggo panelusuran. Bisa ing format 'YYYY-MM-DD' utawa ekspresi relatif kaya 'dina iki', 'wingi'." }, end_date: { type: "STRING", description: "Tanggal pungkasan kanggo panelusuran. Bisa ing format 'YYYY-MM-DD' utawa ekspresi relatif kaya 'dina iki'." } }, required: ["start_date", "end_date"] } }
            ]
        }
    ],
    tl: [
        {
            functionDeclarations: [
                ...
                { name: "schedule_task", description: "Nagtatakda ng email, tala, o file para sa isang partikular na oras sa HINAHARAP. DAPAT piliin ang tool na ito kung ang gumagamit ay gumagamit ng isang ekspresyon ng oras tulad ng 'oras', 'mamaya', 'gabi'.", parameters: { type: "OBJECT", properties: { time: { type: "STRING", description: "Ang oras ng gawain, DAPAT ay nasa 24-oras na format na 'HH:MM' (halimbawa: '17:45')." }, noteName: { type: "STRING", description: "Ang pangalan ng isang paunang na-save na tala na ipapadala sa pamamagitan ng email." }, subject: { type: "STRING", description: "Ang paksa ng isang email na lilikhain mula sa simula." }, body: { type: "STRING", description: "Ang nilalaman ng isang email na lilikhain mula sa simula." }, attachmentDescription: { type: "STRING", description: "Ang paglalarawan ng isang naunang na-save na alaala (file) na ilalakip sa email." } }, required: ["time"] } },
                ...
                { name: "create_calendar_event", description: "Nagdaragdag ng isang bagong kaganapan o appointment sa kalendaryo ng gumagamit.", parameters: { type: "OBJECT", properties: { title: { type: "STRING" }, date: { type: "STRING", description: "Ang petsa ng kaganapan ay DAPAT nasa format na 'YYYY-MM-DD'. Kung sinabi ng gumagamit na 'ngayon' o 'bukas', I-CALCULATE ang kaukulang petsa sa format na ito sa IYONG SARILI. Kung walang petsang tinukoy, gamitin ang araw na ito." }, time: { type: "STRING", description: "Ang oras ng kaganapan ay DAPAT nasa 24-oras na format na 'HH:MM' (halimbawa: '14:30')." }, description: { type: "STRING" } }, required: ["title", "time"] } },
                ...
                { name: "get_crypto_price", description: "Kumukuha ng kasalukuyang presyo ng isang digital currency (cryptocurrency) sa isang partikular na currency.", parameters: { type: "OBJECT", properties: { coin_name: { type: "STRING", description: "Ang pangalan ng cryptocurrency para makuha ang presyo (hal. 'bitcoin', 'ethereum')." }, currency: { type: "STRING", description: "Ang currency kung saan hinihiling ang presyo (hal. 'usd', 'eur', 'try')." } }, required: ["coin_name", "currency"] } },
                { name: "get_stock_price", description: "Kumukuha ng kasalukuyang presyo sa stock market ng stock ng isang kumpanya.", parameters: { type: "OBJECT", properties: { stock_symbol: { type: "STRING", description: "Ang simbolo sa stock market ng stock para makuha ang presyo (hal. 'AAPL', 'TSLA', 'GOOGL')." } }, required: ["stock_symbol"] } },
                { name: "web_search", description: "Naghahanap sa internet para sa isang partikular na paksa at nakakahanap ng mga kasalukuyang resulta. Ginagamit para sa mga paksa tulad ng mga tiket sa eroplano, tiket sa laban, presyo ng hotel, o pangkalahatang impormasyon.", parameters: { type: "OBJECT", properties: { query: { type: "STRING", description: "Ang query na hahanapin sa internet (halimbawa: 'pinakamahusay na mga restawran ng Italyano sa Istanbul')." } }, required: ["query"] } },
                { name: "record_action_receipt", description: "Permanenteng nagse-save ng isang mahalagang aksyon na matagumpay na natapos para sa user (hal., paghahanap ng mga flight, pagtatanong ng mga presyo ng stock, paggawa ng reservation) bilang isang 'resibo' para sa sanggunian sa hinaharap.", parameters: { type: "OBJECT", properties: { summary: { type: "STRING", description: "Isang maikli at malinaw na buod ng aksyon (halimbawa: 'Nakahanap ng Tiket sa Eroplano papuntang Paris', 'Nagtanong ng Presyo ng Stock ng AAPL')." }, action_type: { type: "STRING", description: "Ang uri ng aksyon (halimbawa: 'flight_search', 'stock_price_query', 'hotel_booking')." }, details: { type: "OBJECT", description: "Isang JSON object na naglalaman ng lahat ng detalye ng aksyon. Halimbawa, para sa isang flight: { origin, destination, date, price }." } }, required: ["summary", "action_type", "details"] } },
                { name: "find_receipts", description: "Naghahanap at naglilista ng mga nakaraan at nairekord na aksyon (mga resibo) ng user para sa isang partikular na saklaw ng petsa.", parameters: { type: "OBJECT", properties: { start_date: { type: "STRING", description: "Ang petsa ng pagsisimula ng paghahanap. Maaaring nasa format na 'YYYY-MM-DD' o isang relatibong ekspresyon tulad ng 'ngayon', 'kahapon'." }, end_date: { type: "STRING", description: "Ang petsa ng pagtatapos ng paghahanap. Maaaring nasa format na 'YYYY-MM-DD' o isang relatibong ekspresyon tulad ng 'ngayon'." } }, required: ["start_date", "end_date"] } }
            ]
        }
    ],
    uz: [
        {
            functionDeclarations: [
                ...
                { name: "schedule_task", description: "E-xat, eslatma yoki faylni KELAJAKdagi ma'lum bir vaqtga rejalashtiradi. Agar foydalanuvchi 'soat', 'keyinroq', 'kechqurun' kabi vaqt iboralarini ishlatsa, BU vosita ALBATTA tanlanishi kerak.", parameters: { type: "OBJECT", properties: { time: { type: "STRING", description: "Vazifa vaqti, 24 soatlik 'HH:MM' formatida bo'lishi SHART (misol: '17:45')." }, noteName: { type: "STRING", description: "E-xat orqali yuboriladigan oldindan saqlangan eslatma nomi." }, subject: { type: "STRING", description: "Noldan yaratiladigan e-xat mavzusi." }, body: { type: "STRING", description: "Noldan yaratiladigan e-xat tarkibi." }, attachmentDescription: { type: "STRING", description: "E-xatga ilova qilinadigan oldindan saqlangan xotira (fayl) tavsifi." } }, required: ["time"] } },
                ...
                { name: "create_calendar_event", description: "Foydalanuvchi taqvimiga yangi tadbir yoki uchrashuv qo'shadi.", parameters: { type: "OBJECT", properties: { title: { type: "STRING" }, date: { type: "STRING", description: "Tadbir sanasi 'YYYY-MM-DD' formatida bo'lishi SHART. Agar foydalanuvchi 'bugun' yoki 'ertaga' desa, mos sanani shu formatda O'ZINGIZ HISOBLANG. Agar sana ko'rsatilmasa, bugungi kunni ishlating." }, time: { type: "STRING", description: "Tadbir vaqti, 24 soatlik 'HH:MM' formatida bo'lishi SHART (misol: '14:30')." }, description: { type: "STRING" } }, required: ["title", "time"] } },
                ...
                { name: "get_crypto_price", description: "Raqamli valyutaning (kriptovalyuta) joriy narxini ma'lum bir valyutada oladi.", parameters: { type: "OBJECT", properties: { coin_name: { type: "STRING", description: "Narxini olish uchun kriptovalyuta nomi (masalan, 'bitcoin', 'ethereum')." }, currency: { type: "STRING", description: "Narx so'ralgan valyuta (masalan, 'usd', 'eur', 'try')." } }, required: ["coin_name", "currency"] } },
                { name: "get_stock_price", description: "Kompaniya aksiyasining joriy birja narxini oladi.", parameters: { type: "OBJECT", properties: { stock_symbol: { type: "STRING", description: "Narxini olish uchun aksiya birja belgisi (masalan, 'AAPL', 'TSLA', 'GOOGL')." } }, required: ["stock_symbol"] } },
                { name: "web_search", description: "Ma'lum bir mavzu bo'yicha internetda qidiruv qiladi va joriy natijalarni topadi. Samolyot chiptalari, o'yin chiptalari, mehmonxona narxlari yoki umumiy ma'lumotlar kabi mavzular uchun ishlatiladi.", parameters: { type: "OBJECT", properties: { query: { type: "STRING", description: "Internetda qidiriladigan so'rov (masalan, 'Istanbuldagi eng yaxshi italyan restoranlari')." } }, required: ["query"] } },
                { name: "record_action_receipt", description: "Foydalanuvchi nomidan muvaffaqiyatli bajarilgan muhim harakatni (masalan, reyslarni topish, aksiya narxlarini so'rash, bron qilish) kelajakda foydalanish uchun 'kvitansiya' sifatida doimiy saqlaydi.", parameters: { type: "OBJECT", properties: { summary: { type: "STRING", description: "Harakatning qisqa va tushunarli xulosasi (masalan, 'Parijga aviachipta topildi', 'AAPL aksiyasi narxi so'raldi')." }, action_type: { type: "STRING", description: "Harakat turi (masalan, 'flight_search', 'stock_price_query', 'hotel_booking')." }, details: { type: "OBJECT", description: "Harakatning barcha tafsilotlarini o'z ichiga olgan JSON ob'ekti. Masalan, reys uchun: { origin, destination, date, price }." } }, required: ["summary", "action_type", "details"] } },
                { name: "find_receipts", description: "Foydalanuvchining o'tgan va qayd etilgan harakatlarini (kvitansiyalarni) ma'lum bir sana oralig'i uchun topadi.", parameters: { type: "OBJECT", properties: { start_date: { type: "STRING", description: "Qidiruvning boshlanish sanasi. 'YYYY-MM-DD' formatida yoki 'bugun', 'kecha' kabi nisbiy ibora bo'lishi mumkin." }, end_date: { type: "STRING", description: "Qidiruvning tugash sanasi. 'YYYY-MM-DD' formatida yoki 'bugun' kabi nisbiy ibora bo'lishi mumkin." } }, required: ["start_date", "end_date"] } }
            ]
        }
    ],
    nl: [
        {
            functionDeclarations: [
                ...
                { name: "schedule_task", description: "Plant een e-mail, notitie of bestand voor een specifieke tijd in de TOEKOMST. Deze tool MOET worden gekozen als de gebruiker een tijdsuitdrukking gebruikt zoals 'uur', 'later', 'avond'.", parameters: { type: "OBJECT", properties: { time: { type: "STRING", description: "De tijd van de taak MOET in 24-uurs 'HH:MM' formaat zijn (voorbeeld: '17:45')." }, noteName: { type: "STRING", description: "De naam van een vooraf opgeslagen notitie die per e-mail wordt verzonden." }, subject: { type: "STRING", description: "Het onderwerp van een e-mail die van begin af aan wordt gemaakt." }, body: { type: "STRING", description: "De inhoud van een e-mail die van begin af aan wordt gemaakt." }, attachmentDescription: { type: "STRING", description: "De beschrijving van een eerder opgeslagen herinnering (bestand) die aan de e-mail moet worden toegevoegd." } }, required: ["time"] } },
                ...
                { name: "create_calendar_event", description: "Voegt een nieuw evenement of afspraak toe aan de kalender van de gebruiker.", parameters: { type: "OBJECT", properties: { title: { type: "STRING" }, date: { type: "STRING", description: "De datum van het evenement MOET in 'YYYY-MM-DD' formaat zijn. Als de gebruiker 'vandaag' of 'morgen' zegt, BEREKEN dan ZELF de overeenkomstige datum in dit formaat. Gebruik vandaag als er geen datum is opgegeven." }, time: { type: "STRING", description: "De tijd van het evenement MOET in 24-uurs 'HH:MM' formaat zijn (voorbeeld: '14:30')." }, description: { type: "STRING" } }, required: ["title", "time"] } },
                ...
                { name: "get_crypto_price", description: "Haalt de huidige prijs van een digitale valuta (cryptocurrency) op in een specifieke valuta.", parameters: { type: "OBJECT", properties: { coin_name: { type: "STRING", description: "De naam van de cryptocurrency om de prijs van op te halen (bijv. 'bitcoin', 'ethereum')." }, currency: { type: "STRING", description: "De valuta waarin de prijs wordt opgevraagd (bijv. 'usd', 'eur', 'try')." } }, required: ["coin_name", "currency"] } },
                { name: "get_stock_price", description: "Haalt de huidige beurskoers van het aandeel van een bedrijf op.", parameters: { type: "OBJECT", properties: { stock_symbol: { type: "STRING", description: "Het beurssymbool van het aandeel om de prijs van op te halen (bijv. 'AAPL', 'TSLA', 'GOOGL')." } }, required: ["stock_symbol"] } },
                { name: "web_search", description: "Zoekt op internet naar een specifiek onderwerp en vindt actuele resultaten. Wordt gebruikt voor onderwerpen als vliegtickets, wedstrijdkaartjes, hotelprijzen of algemene informatie.", parameters: { type: "OBJECT", properties: { query: { type: "STRING", description: "De zoekopdracht die op internet moet worden gezocht (bijvoorbeeld: 'beste Italiaanse restaurants in Amsterdam')." } }, required: ["query"] } },
                { name: "record_action_receipt", description: "Slaat een belangrijke, succesvol voltooide actie namens de gebruiker (bijv. vluchten zoeken, aandelenkoersen opvragen, een reservering maken) permanent op als een 'ontvangstbewijs' voor toekomstig gebruik.", parameters: { type: "OBJECT", properties: { summary: { type: "STRING", description: "Een korte en duidelijke samenvatting van de actie (voorbeeld: 'Vluchtticket naar Parijs gevonden', 'Aandelenkoers AAPL opgevraagd')." }, action_type: { type: "STRING", description: "Het type actie (voorbeeld: 'flight_search', 'stock_price_query', 'hotel_booking')." }, details: { type: "OBJECT", description: "Een JSON-object met alle details van de actie. Bijvoorbeeld voor een vlucht: { origin, destination, date, price }." } }, required: ["summary", "action_type", "details"] } },
                { name: "find_receipts", description: "Vindt de eerdere en opgeslagen acties (ontvangstbewijzen) van de gebruiker voor een specifieke datumperiode.", parameters: { type: "OBJECT", properties: { start_date: { type: "STRING", description: "De startdatum voor de zoekopdracht. Kan in 'YYYY-MM-DD' formaat zijn of een relatieve uitdrukking zoals 'vandaag', 'gisteren'." }, end_date: { type: "STRING", description: "De einddatum voor de zoekopdracht. Kan in 'YYYY-MM-DD' formaat zijn of een relatieve uitdrukking zoals 'vandaag'." } }, required: ["start_date", "end_date"] } }
            ]
        }
    ],
    el: [
        {
            functionDeclarations: [
                ...
                { name: "schedule_task", description: "Προγραμματίζει ένα email, μια σημείωση ή ένα αρχείο για μια συγκεκριμένη ώρα στο ΜΕΛΛΟΝ. Αυτό το εργαλείο ΠΡΕΠΕΙ να επιλεγεί εάν ο χρήστης χρησιμοποιεί μια έκφραση χρόνου όπως 'ώρα', 'αργότερα', 'βράδυ'.", parameters: { type: "OBJECT", properties: { time: { type: "STRING", description: "Η ώρα της εργασίας ΠΡΕΠΕΙ να είναι σε 24ωρη μορφή 'HH:MM' (παράδειγμα: '17:45')." }, noteName: { type: "STRING", description: "Το όνομα μιας προ-αποθηκευμένης σημείωσης που θα σταλεί μέσω email." }, subject: { type: "STRING", description: "Το θέμα ενός email που θα δημιουργηθεί από την αρχή." }, body: { type: "STRING", description: "Το περιεχόμενο ενός email που θα δημιουργηθεί από την αρχή." }, attachmentDescription: { type: "STRING", description: "Η περιγραφή μιας προηγουμένως αποθηκευμένης ανάμνησης (αρχείου) που θα επισυναφθεί στο email." } }, required: ["time"] } },
                ...
                { name: "create_calendar_event", description: "Προσθέτει ένα νέο γεγονός ή ραντεβού στο ημερολόγιο του χρήστη.", parameters: { type: "OBJECT", properties: { title: { type: "STRING" }, date: { type: "STRING", description: "Η ημερομηνία του γεγονότος ΠΡΕΠΕΙ να είναι σε μορφή 'YYYY-MM-DD'. Εάν ο χρήστης πει 'σήμερα' ή 'αύριο', ΥΠΟΛΟΓΙΣΤΕ ΜΟΝΟΙ ΣΑΣ την αντίστοιχη ημερομηνία σε αυτήν τη μορφή. Εάν δεν καθοριστεί ημερομηνία, χρησιμοποιήστε τη σημερινή." }, time: { type: "STRING", description: "Η ώρα του γεγονότος ΠΡΕΠΕΙ να είναι σε 24ωρη μορφή 'HH:MM' (παράδειγμα: '14:30')." }, description: { type: "STRING" } }, required: ["title", "time"] } },
                ...
                { name: "get_crypto_price", description: "Λαμβάνει την τρέχουσα τιμή ενός ψηφιακού νομίσματος (κρυπτονόμισμα) σε ένα συγκεκριμένο νόμισμα.", parameters: { type: "OBJECT", properties: { coin_name: { type: "STRING", description: "Το όνομα του κρυπτονομίσματος για το οποίο θα ληφθεί η τιμή (π.χ. 'bitcoin', 'ethereum')." }, currency: { type: "STRING", description: "Το νόμισμα στο οποίο ζητείται η τιμή (π.χ. 'usd', 'eur', 'try')." } }, required: ["coin_name", "currency"] } },
                { name: "get_stock_price", description: "Λαμβάνει την τρέχουσα χρηματιστηριακή τιμή της μετοχής μιας εταιρείας.", parameters: { type: "OBJECT", properties: { stock_symbol: { type: "STRING", description: "Το χρηματιστηριακό σύμβολο της μετοχής για την οποία θα ληφθεί η τιμή (π.χ. 'AAPL', 'TSLA', 'GOOGL')." } }, required: ["stock_symbol"] } },
                { name: "web_search", description: "Αναζητά στο διαδίκτυο για ένα συγκεκριμένο θέμα και βρίσκει τρέχοντα αποτελέσματα. Χρησιμοποιείται για θέματα όπως αεροπορικά εισιτήρια, εισιτήρια αγώνων, τιμές ξενοδοχείων ή γενικές πληροφορίες.", parameters: { type: "OBJECT", properties: { query: { type: "STRING", description: "Το ερώτημα που θα αναζητηθεί στο διαδίκτυο (παράδειγμα: 'τα καλύτερα ιταλικά εστιατόρια στην Κωνσταντινούπολη')." } }, required: ["query"] } },
                { name: "record_action_receipt", description: "Αποθηκεύει μόνιμα μια σημαντική ενέργεια που ολοκληρώθηκε με επιτυχία εκ μέρους του χρήστη (π.χ. εύρεση πτήσεων, αναζήτηση τιμών μετοχών, πραγματοποίηση κράτησης) ως 'απόδειξη' για μελλοντική αναφορά.", parameters: { type: "OBJECT", properties: { summary: { type: "STRING", description: "Μια σύντομη και σαφής περίληψη της ενέργειας (παράδειγμα: 'Βρέθηκε αεροπορικό εισιτήριο για Παρίσι', 'Αναζητήθηκε η τιμή της μετοχής AAPL')." }, action_type: { type: "STRING", description: "Ο τύπος της ενέργειας (παράδειγμα: 'flight_search', 'stock_price_query', 'hotel_booking')." }, details: { type: "OBJECT", description: "Ένα αντικείμενο JSON που περιέχει όλες τις λεπτομέρειες της ενέργειας. Για παράδειγμα, για μια πτήση: { origin, destination, date, price }." } }, required: ["summary", "action_type", "details"] } },
                { name: "find_receipts", description: "Βρίσκει και παραθέτει τις προηγούμενες και καταγεγραμμένες ενέργειες του χρήστη (αποδείξεις) για ένα συγκεκριμένο εύρος ημερομηνιών.", parameters: { type: "OBJECT", properties: { start_date: { type: "STRING", description: "Η ημερομηνία έναρξης της αναζήτησης. Μπορεί να είναι σε μορφή 'YYYY-MM-DD' ή μια σχετική έκφραση όπως 'σήμερα', 'χθες'." }, end_date: { type: "STRING", description: "Η ημερομηνία λήξης της αναζήτησης. Μπορεί να είναι σε μορφή 'YYYY-MM-DD' ή μια σχετική έκφραση όπως 'σήμερα'." } }, required: ["start_date", "end_date"] } }
            ]
        }
    ],
    sv: [
        {
            functionDeclarations: [
                ...
                { name: "schedule_task", description: "Schemalägger ett e-postmeddelande, en anteckning eller en fil för en specifik tid i FRAMTIDEN. Detta verktyg MÅSTE väljas om användaren använder ett tidsuttryck som 'timme', 'senare', 'kväll'.", parameters: { type: "OBJECT", properties: { time: { type: "STRING", description: "Tiden för uppgiften MÅSTE vara i 24-timmarsformat 'HH:MM' (exempel: '17:45')." }, noteName: { type: "STRING", description: "Namnet på en försparad anteckning som ska skickas via e-post." }, subject: { type: "STRING", description: "Ämnet för ett e-postmeddelande som ska skapas från grunden." }, body: { type: "STRING", description: "Innehållet i ett e-postmeddelande som ska skapas från grunden." }, attachmentDescription: { type: "STRING", description: "Beskrivningen av ett tidigare sparat minne (fil) som ska bifogas e-postmeddelandet." } }, required: ["time"] } },
                ...
                { name: "create_calendar_event", description: "Lägger till en ny händelse eller ett nytt möte i användarens kalender.", parameters: { type: "OBJECT", properties: { title: { type: "STRING" }, date: { type: "STRING", description: "Händelsedatum MÅSTE vara i formatet 'YYYY-MM-DD'. Om användaren säger 'idag' eller 'imorgon', BERÄKNA motsvarande datum i detta format SJÄLV. Om inget datum anges, använd dagens datum." }, time: { type: "STRING", description: "Händelsetid MÅSTE vara i 24-timmarsformat 'HH:MM' (exempel: '14:30')." }, description: { type: "STRING" } }, required: ["title", "time"] } },
                ...
                { name: "get_crypto_price", description: "Hämtar det aktuella priset på en digital valuta (kryptovaluta) i en specifik valuta.", parameters: { type: "OBJECT", properties: { coin_name: { type: "STRING", description: "Namnet på kryptovalutan för att hämta priset (t.ex. 'bitcoin', 'ethereum')." }, currency: { type: "STRING", description: "Valutan som priset begärs i (t.ex. 'usd', 'eur', 'sek')." } }, required: ["coin_name", "currency"] } },
                { name: "get_stock_price", description: "Hämtar det aktuella börspriset på ett företags aktie.", parameters: { type: "OBJECT", properties: { stock_symbol: { type: "STRING", description: "Börssymbolen för aktien för att hämta priset (t.ex. 'AAPL', 'TSLA', 'GOOGL')." } }, required: ["stock_symbol"] } },
                { name: "web_search", description: "Söker på internet efter ett specifikt ämne och hittar aktuella resultat. Används för ämnen som flygbiljetter, matchbiljetter, hotellpriser eller allmän information.", parameters: { type: "OBJECT", properties: { query: { type: "STRING", description: "Sökfrågan som ska sökas på internet (exempel: 'bästa italienska restauranger i Stockholm')." } }, required: ["query"] } },
                { name: "record_action_receipt", description: "Sparar permanent en viktig åtgärd som slutförts framgångsrikt för användarens räkning (t.ex. hitta flyg, fråga aktiekurs, göra en bokning) som ett 'kvitto' för framtida referens.", parameters: { type: "OBJECT", properties: { summary: { type: "STRING", description: "En kort och tydlig sammanfattning av handlingen (exempel: 'Flygbiljett till Paris hittades', 'AAPL Aktiekurs efterfrågades')." }, action_type: { type: "STRING", description: "Typen av handling (exempel: 'flight_search', 'stock_price_query', 'hotel_booking')." }, details: { type: "OBJECT", description: "Ett JSON-objekt som innehåller alla detaljer om handlingen. Till exempel för ett flyg: { origin, destination, date, price }." } }, required: ["summary", "action_type", "details"] } },
                { name: "find_receipts", description: "Hittar och listar användarens tidigare och registrerade åtgärder (kvitton) för ett specifikt datumintervall.", parameters: { type: "OBJECT", properties: { start_date: { type: "STRING", description: "Startdatum för sökningen. Kan vara i 'YYYY-MM-DD'-format eller ett relativt uttryck som 'idag', 'igår'." }, end_date: { type: "STRING", description: "Slutdatum för sökningen. Kan vara i 'YYYY-MM-DD'-format eller ett relativt uttryck som 'idag'." } }, required: ["start_date", "end_date"] } }
            ]
        }
    ],
    he: [
        {
            functionDeclarations: [
                ...
                { name: "schedule_task", description: "מתזמן אימייל, פתק או קובץ לשעה ספציפית בעתיד. יש לבחור כלי זה בהכרח אם המשתמש משתמש בביטוי זמן כמו 'שעה', 'אחר כך', 'ערב'.", parameters: { type: "OBJECT", properties: { time: { type: "STRING", description: "זמן המשימה חייב להיות בפורמט 24 שעות 'HH:MM' (דוגמה: '17:45')." }, noteName: { type: "STRING", description: "שם של פתק שנשמר מראש ויישלח באימייל." }, subject: { type: "STRING", description: "הנושא של אימייל שייווצר מאפס." }, body: { type: "STRING", description: "התוכן של אימייל שייווצר מאפס." }, attachmentDescription: { type: "STRING", description: "תיאור של זיכרון (קובץ) שנשמר בעבר ושיצורף לאימייל." } }, required: ["time"] } },
                ...
                { name: "create_calendar_event", description: "מוסיף אירוע או פגישה חדשים ליומן של המשתמש.", parameters: { type: "OBJECT", properties: { title: { type: "STRING" }, date: { type: "STRING", description: "תאריך האירוע חייב להיות בפורמט 'YYYY-MM-DD'. אם המשתמש אומר 'היום' או 'מחר', חשב בעצמך את התאריך המתאים בפורמט זה. אם לא צוין תאריך, השתמש בהיום." }, time: { type: "STRING", description: "שעת האירוע חייבת להיות בפורמט 24 שעות 'HH:MM' (דוגמה: '14:30')." }, description: { type: "STRING" } }, required: ["title", "time"] } },
                ...
                { name: "get_crypto_price", description: "מקבל את המחיר הנוכחי של מטבע דיגיטלי (מטבע קריפטוגרפי) במטבע מסוים.", parameters: { type: "OBJECT", properties: { coin_name: { type: "STRING", description: "שם המטבע הקריפטוגרפי לקבלת המחיר (לדוגמה: 'bitcoin', 'ethereum')." }, currency: { type: "STRING", description: "המטבע שבו מתבקש המחיר (לדוגמה: 'usd', 'eur', 'ils')." } }, required: ["coin_name", "currency"] } },
                { name: "get_stock_price", description: "מקבל את מחיר המניה הנוכחי של חברה.", parameters: { type: "OBJECT", properties: { stock_symbol: { type: "STRING", description: "סמל המניה לקבלת המחיר (לדוגמה: 'AAPL', 'TSLA', 'GOOGL')." } }, required: ["stock_symbol"] } },
                { name: "web_search", description: "מחפש באינטרנט נושא מסוים ומוצא תוצאות עדכניות. משמש לנושאים כמו כרטיסי טיסה, כרטיסים למשחקים, מחירי מלונות או מידע כללי.", parameters: { type: "OBJECT", properties: { query: { type: "STRING", description: "השאילתה לחיפוש באינטרנט (דוגמה: 'המסעדות האיטלקיות הטובות ביותר בתל אביב')." } }, required: ["query"] } },
                { name: "record_action_receipt", description: "שומר באופן קבוע פעולה חשובה שהושלמה בהצלחה בשם המשתמש (כמו מציאת טיסות, בירור מחירי מניות, ביצוע הזמנה וכו') כ'קבלה' לעיון עתידי.", parameters: { type: "OBJECT", properties: { summary: { type: "STRING", description: "סיכום קצר וברור של הפעולה (דוגמה: 'נמצא כרטיס טיסה לפריז', 'נשאלה שאילתה על מחיר מניית AAPL')." }, action_type: { type: "STRING", description: "סוג הפעולה (דוגמה: 'flight_search', 'stock_price_query', 'hotel_booking')." }, details: { type: "OBJECT", description: "אובייקט JSON המכיל את כל פרטי הפעולה. לדוגמה, עבור טיסה: { origin, destination, date, price }." } }, required: ["summary", "action_type", "details"] } },
                { name: "find_receipts", description: "מוצא ומציג את הפעולות הקודמות והמתועדות של המשתמש (קבלות) עבור טווח תאריכים מסוים.", parameters: { type: "OBJECT", properties: { start_date: { type: "STRING", description: "תאריך ההתחלה של החיפוש. יכול להיות בפורמט 'YYYY-MM-DD' או ביטוי יחסי כמו 'היום', 'אתמול'." }, end_date: { type: "STRING", description: "תאריך הסיום של החיפוש. יכול להיות בפורמט 'YYYY-MM-DD' או ביטוי יחסי כמו 'היום'." } }, required: ["start_date", "end_date"] } }
            ]
        }
    ],
    da: [
        {
            functionDeclarations: [
                ...
                { name: "schedule_task", description: "Planlægger en e-mail, en note eller en fil til et bestemt tidspunkt i FREMTIDEN. Dette værktøj SKAL vælges, hvis brugeren anvender et tidsudtryk som 'time', 'senere', 'aften'.", parameters: { type: "OBJECT", properties: { time: { type: "STRING", description: "Tidspunktet for opgaven SKAL være i 24-timers 'HH:MM' format (eksempel: '17:45')." }, noteName: { type: "STRING", description: "Navnet på en tidligere gemt note, der skal sendes via e-mail." }, subject: { type: "STRING", description: "Emnet for en e-mail, der skal oprettes fra bunden." }, body: { type: "STRING", description: "Indholdet af en e-mail, der skal oprettes fra bunden." }, attachmentDescription: { type: "STRING", description: "En beskrivelse af et tidligere gemt minde (fil), der skal vedhæftes e-mailen." } }, required: ["time"] } },
                ...
                { name: "create_calendar_event", description: "Tilføjer en ny begivenhed eller aftale til brugerens kalender.", parameters: { type: "OBJECT", properties: { title: { type: "STRING" }, date: { type: "STRING", description: "Begivenhedsdatoen SKAL være i 'YYYY-MM-DD' format. Hvis brugeren siger 'i dag' eller 'i morgen', BEREGN den tilsvarende dato i dette format SELV. Hvis ingen dato er angivet, brug dags dato." }, time: { type: "STRING", description: "Begivenhedstidspunktet SKAL være i 24-timers 'HH:MM' format (eksempel: '14:30')." }, description: { type: "STRING" } }, required: ["title", "time"] } },
                ...
                { name: "get_crypto_price", description: "Henter den aktuelle pris på en digital valuta (kryptovaluta) i en bestemt valuta.", parameters: { type: "OBJECT", properties: { coin_name: { type: "STRING", description: "Navnet på den kryptovaluta, der skal hentes pris for (f.eks. 'bitcoin', 'ethereum')." }, currency: { type: "STRING", description: "Valutaen, som prisen anmodes i (f.eks. 'usd', 'eur', 'dkk')." } }, required: ["coin_name", "currency"] } },
                { name: "get_stock_price", description: "Henter den aktuelle børskurs på en virksomheds aktie.", parameters: { type: "OBJECT", properties: { stock_symbol: { type: "STRING", description: "Børssymbolet for den aktie, der skal hentes pris for (f.eks. 'AAPL', 'TSLA', 'GOOGL')." } }, required: ["stock_symbol"] } },
                { name: "web_search", description: "Søger på internettet efter et bestemt emne og finder aktuelle resultater. Bruges til emner som flybilletter, kampbilletter, hotelpriser eller generel information.", parameters: { type: "OBJECT", properties: { query: { type: "STRING", description: "Søgeforespørgslen, der skal søges på internettet (eksempel: 'bedste italienske restauranter i København')." } }, required: ["query"] } },
                { name: "record_action_receipt", description: "Gemmer permanent en vigtig handling, der er fuldført med succes på vegne af brugeren (f.eks. finde fly, forespørge aktiekurs, foretage en reservation) som en 'kvittering' til fremtidig reference.", parameters: { type: "OBJECT", properties: { summary: { type: "STRING", description: "En kort og klar opsummering af handlingen (eksempel: 'Flybillet til Paris fundet', 'AAPL Aktiekurs forespurgt')." }, action_type: { type: "STRING", description: "Typen af handling (eksempel: 'flight_search', 'stock_price_query', 'hotel_booking')." }, details: { type: "OBJECT", description: "Et JSON-objekt, der indeholder alle detaljer om handlingen. F.eks. for et fly: { origin, destination, date, price }." } }, required: ["summary", "action_type", "details"] } },
                { name: "find_receipts", description: "Finder og lister brugerens tidligere og registrerede handlinger (kvitteringer) for et bestemt datointerval.", parameters: { type: "OBJECT", properties: { start_date: { type: "STRING", description: "Startdatoen for søgningen. Kan være i 'YYYY-MM-DD' format eller et relativt udtryk som 'i dag', 'i går'." }, end_date: { type: "STRING", description: "Slutdatoen for søgningen. Kan være i 'YYYY-MM-DD' format eller et relativt udtryk som 'i dag'." } }, required: ["start_date", "end_date"] } }
            ]
        }
    ],
    fi: [
        {
            functionDeclarations: [
                ...
                { name: "schedule_task", description: "Aikatauluttaa sähköpostin, muistiinpanon tai tiedoston tiettyyn aikaan TULEVAISUUDESSA. Tämä työkalu ON valittava, jos käyttäjä käyttää ajan ilmaisua kuten 'tunti', 'myöhemmin', 'ilta'.", parameters: { type: "OBJECT", properties: { time: { type: "STRING", description: "Tehtävän ajan TÄYTYY olla 24 tunnin 'HH:MM'-muodossa (esimerkki: '17:45')." }, noteName: { type: "STRING", description: "Sähköpostitse lähetettävän, aiemmin tallennetun muistiinpanon nimi." }, subject: { type: "STRING", description: "Tyhjästä luotavan sähköpostin aihe." }, body: { type: "STRING", description: "Tyhjästä luotavan sähköpostin sisältö." }, attachmentDescription: { type: "STRING", description: "Sähköpostiin liitettävän, aiemmin tallennetun muiston (tiedoston) kuvaus." } }, required: ["time"] } },
                ...
                { name: "create_calendar_event", description: "Lisää uuden tapahtuman tai tapaamisen käyttäjän kalenteriin.", parameters: { type: "OBJECT", properties: { title: { type: "STRING" }, date: { type: "STRING", description: "Tapahtuman päivämäärän TÄYTYY olla 'YYYY-MM-DD'-muodossa. Jos käyttäjä sanoo 'tänään' tai 'huomenna', LASKE vastaava päivämäärä tässä muodossa ITSE. Jos päivämäärää ei ole määritetty, käytä tätä päivää." }, time: { type: "STRING", description: "Tapahtuman ajan TÄYTYY olla 24 tunnin 'HH:MM'-muodossa (esimerkki: '14:30')." }, description: { type: "STRING" } }, required: ["title", "time"] } },
                ...
                { name: "get_crypto_price", description: "Hakee digitaalisen valuutan (kryptovaluutan) nykyisen hinnan tietyssä valuutassa.", parameters: { type: "OBJECT", properties: { coin_name: { type: "STRING", description: "Sen kryptovaluutan nimi, jonka hinta haetaan (esim. 'bitcoin', 'ethereum')." }, currency: { type: "STRING", description: "Valuutta, jossa hinta pyydetään (esim. 'usd', 'eur', 'try')." } }, required: ["coin_name", "currency"] } },
                { name: "get_stock_price", description: "Hakee yrityksen osakkeen nykyisen pörssikurssin.", parameters: { type: "OBJECT", properties: { stock_symbol: { type: "STRING", description: "Sen osakkeen pörssitunnus, jonka hinta haetaan (esim. 'AAPL', 'TSLA', 'GOOGL')." } }, required: ["stock_symbol"] } },
                { name: "web_search", description: "Hakee internetistä tietoa tietystä aiheesta ja löytää ajankohtaisia tuloksia. Käytetään aiheisiin kuten lentoliput, otteluliput, hotellihinnat tai yleistieto.", parameters: { type: "OBJECT", properties: { query: { type: "STRING", description: "Internetistä haettava kysely (esimerkki: 'Helsingin parhaat italialaiset ravintolat')." } }, required: ["query"] } },
                { name: "record_action_receipt", description: "Tallentaa pysyvästi tärkeän, käyttäjän puolesta onnistuneesti suoritetun toimenpiteen (esim. lentojen haku, osakekurssin kysely, varauksen tekeminen) 'kuitiksi' tulevaa käyttöä varten.", parameters: { type: "OBJECT", properties: { summary: { type: "STRING", description: "Toiminnon lyhyt ja selkeä yhteenveto (esimerkki: 'Lentolippu Pariisiin löydetty', 'AAPL:n osakekurssi kysytty')." }, action_type: { type: "STRING", description: "Toiminnon tyyppi (esimerkki: 'flight_search', 'stock_price_query', 'hotel_booking')." }, details: { type: "OBJECT", description: "JSON-objekti, joka sisältää kaikki toiminnon yksityiskohdat. Esimerkiksi lennolle: { origin, destination, date, price }." } }, required: ["summary", "action_type", "details"] } },
                { name: "find_receipts", description: "Etsii ja listaa käyttäjän aiemmat ja tallennetut toiminnot (kuitit) tietyltä aikaväliltä.", parameters: { type: "OBJECT", properties: { start_date: { type: "STRING", description: "Haun alkamispäivä. Voi olla 'YYYY-MM-DD'-muodossa tai suhteellinen ilmaus kuten 'tänään', 'eilen'." }, end_date: { type: "STRING", description: "Haun päättymispäivä. Voi olla 'YYYY-MM-DD'-muodossa tai suhteellinen ilmaus kuten 'tänään'." } }, required: ["start_date", "end_date"] } }
            ]
        }
    ],
    no: [
        {
            functionDeclarations: [
                ...
                { name: "schedule_task", description: "Planlegger en e-post, et notat eller en fil for et bestemt tidspunkt i FREMTIDEN. Dette verktøyet MÅ velges hvis brukeren bruker et tidsuttrykk som 'time', 'senere', 'kveld'.", parameters: { type: "OBJECT", properties: { time: { type: "STRING", description: "Tidspunktet for oppgaven MÅ være i 24-timersformatet 'HH:MM' (eksempel: '17:45')." }, noteName: { type: "STRING", description: "Navnet på et forhåndslagret notat som skal sendes via e-post." }, subject: { type: "STRING", description: "Emnet for en e-post som skal opprettes fra bunnen av." }, body: { type: "STRING", description: "Innholdet i en e-post som skal opprettes fra bunnen av." }, attachmentDescription: { type: "STRING", description: "Beskrivelsen av et tidligere lagret minne (fil) som skal legges ved e-posten." } }, required: ["time"] } },
                ...
                { name: "create_calendar_event", description: "Legger til en ny hendelse eller avtale i brukerens kalender.", parameters: { type: "OBJECT", properties: { title: { type: "STRING" }, date: { type: "STRING", description: "Datoen for hendelsen MÅ være i formatet 'YYYY-MM-DD'. Hvis brukeren sier 'i dag' eller 'i morgen', REGN UT den tilsvarende datoen i dette formatet SELV. Hvis ingen dato er angitt, bruk dagens dato." }, time: { type: "STRING", description: "Tidspunktet for hendelsen MÅ være i 24-timersformatet 'HH:MM' (eksempel: '14:30')." }, description: { type: "STRING" } }, required: ["title", "time"] } },
                ...
                { name: "get_crypto_price", description: "Henter gjeldende pris på en digital valuta (kryptovaluta) i en bestemt valuta.", parameters: { type: "OBJECT", properties: { coin_name: { type: "STRING", description: "Navnet på kryptovalutaen for å hente prisen (f.eks. 'bitcoin', 'ethereum')." }, currency: { type: "STRING", description: "Valutaen prisen er forespurt i (f.eks. 'usd', 'eur', 'nok')." } }, required: ["coin_name", "currency"] } },
                { name: "get_stock_price", description: "Henter gjeldende aksjekurs for et selskaps aksje.", parameters: { type: "OBJECT", properties: { stock_symbol: { type: "STRING", description: "Aksjesymbolet for aksjen for å hente prisen (f.eks. 'AAPL', 'TSLA', 'GOOGL')." } }, required: ["stock_symbol"] } },
                { name: "web_search", description: "Søker på internett etter et bestemt emne og finner aktuelle resultater. Brukes for emner som flybilletter, kampbilletter, hotellpriser eller generell informasjon.", parameters: { type: "OBJECT", properties: { query: { type: "STRING", description: "Søket som skal utføres på internett (eksempel: 'beste italienske restauranter i Oslo')." } }, required: ["query"] } },
                { name: "record_action_receipt", description: "Lagrer permanent en viktig handling som er fullført på vegne av brukeren (f.eks. finne flyreiser, sjekke aksjekurser, gjøre en bestilling) som en 'kvittering' for fremtidig referanse.", parameters: { type: "OBJECT", properties: { summary: { type: "STRING", description: "En kort og forståelig oppsummering av handlingen (eksempel: 'Flybillett til Paris funnet', 'AAPL Aksjekurs sjekket')." }, action_type: { type: "STRING", description: "Typen handling (eksempel: 'flight_search', 'stock_price_query', 'hotel_booking')." }, details: { type: "OBJECT", description: "Et JSON-objekt som inneholder alle detaljer om handlingen. For eksempel for en flyreise: { origin, destination, date, price }." } }, required: ["summary", "action_type", "details"] } },
                { name: "find_receipts", description: "Finner og lister brukerens tidligere og registrerte handlinger (kvitteringer) for et bestemt datointervall.", parameters: { type: "OBJECT", properties: { start_date: { type: "STRING", description: "Startdatoen for søket. Kan være i 'YYYY-MM-DD'-format eller et relativt uttrykk som 'i dag', 'i går'." }, end_date: { type: "STRING", description: "Sluttdatoen for søket. Kan være i 'YYYY-MM-DD'-format eller et relativt uttrykk som 'i dag'." } }, required: ["start_date", "end_date"] } }
            ]
        }
    ]
};
...
const translations = {
    tr: {
        ...
        "crypto_price_queried": "'{coin_name}' fiyatı başarıyla sorgulandı",
        "stock_price_queried": "'{stock_symbol}' hisse senedi fiyatı başarıyla sorgulandı"
    },
    en: {
        ...
        "crypto_price_queried": "Price of '{coin_name}' was successfully queried",
        "stock_price_queried": "Stock price for '{stock_symbol}' was successfully queried"
    },
    zh: {
        "welcome": "你好！我能帮什么忙？",
        ...
        "crypto_price_queried": "'{coin_name}' 的价格查询成功",
        "stock_price_queried": "'{stock_symbol}' 的股价查询成功"
    },
    hi: {
        ...
        "crypto_price_queried": "'{coin_name}' की कीमत सफलतापूर्वक पूछी गई",
        "stock_price_queried": "'{stock_symbol}' के स्टॉक की कीमत सफलतापूर्वक पूछी गई"
    },
    es: {
        ...
        "crypto_price_queried": "El precio de '{coin_name}' se consultó con éxito",
        "stock_price_queried": "El precio de la acción '{stock_symbol}' se consultó con éxito"
    },
    fr: {
        ...
        "crypto_price_queried": "Le prix de '{coin_name}' a été interrogé avec succès",
        "stock_price_queried": "Le cours de l'action '{stock_symbol}' a été interrogé avec succès"
    },
    ar: {
        ...
        "crypto_price_queried": "تم الاستعلام عن سعر '{coin_name}' بنجاح",
        "stock_price_queried": "تم الاستعلام عن سعر سهم '{stock_symbol}' بنجاح"
    },
    bn: {
        ...
        "crypto_price_queried": "'{coin_name}' এর মূল্য সফলভাবে জিজ্ঞাসা করা হয়েছে",
        "stock_price_queried": "'{stock_symbol}' এর শেয়ারের মূল্য সফলভাবে জিজ্ঞাসা করা হয়েছে"
    },
    ru: {
        ...
        "crypto_price_queried": "Цена '{coin_name}' успешно запрошена",
        "stock_price_queried": "Цена акции '{stock_symbol}' успешно запрошена"
    },
    pt: {
        ...
        "crypto_price_queried": "O preço de '{coin_name}' foi consultado com sucesso",
        "stock_price_queried": "O preço da ação '{stock_symbol}' foi consultado com sucesso"
    },
    ur: {
        ...
        "memory_search_not_found": "'{searchText}' سے مماثل کوئی میমوری نہیں ملی۔",
        ...
        "crypto_price_queried": "'{coin_name}' کی قیمت کامیابی سے دریافت کی گئی",
        "stock_price_queried": "'{stock_symbol}' اسٹاک کی قیمت کامیابی سے دریافت کی گئی"
    },
    ms: {
        ...
        "invalid_ai_response": "Respons không hợp lệ nhận được từ AI",
        "chatCleared": "Sembang dikosongkan. Anda boleh memulakan perbualan baharu.",
        "crypto_price_queried": "Harga '{coin_name}' berjaya ditanya",
        "stock_price_queried": "Harga saham untuk '{stock_symbol}' berjaya ditanya"
    },
    de: {
        ...
        "crypto_price_queried": "Preis von '{coin_name}' wurde erfolgreich abgefragt",
        "stock_price_queried": "Aktienkurs für '{stock_symbol}' wurde erfolgreich abgefragt"
    },
    ja: {
        ...
        "crypto_price_queried": "'{coin_name}'の価格が正常に照会されました",
        "stock_price_queried": "'{stock_symbol}'の株価が正常に照会されました"
    },
    fa: {
        ...
        "crypto_price_queried": "قیمت '{coin_name}' با موفقیت استعلام شد",
        "stock_price_queried": "قیمت سهام '{stock_symbol}' با موفقیت استعلام شد"
    },
    ha: {
        ...
        "crypto_price_queried": "An yi nasarar tambayar farashin '{coin_name}'",
        "stock_price_queried": "An yi nasarar tambayar farashin hannun jari na '{stock_symbol}'"
    },
    sw: {
        ...
        "crypto_price_queried": "Bei ya '{coin_name}' imeulizwa kwa mafanikio",
        "stock_price_queried": "Bei ya hisa ya '{stock_symbol}' imeulizwa kwa mafanikio"
    },
    vi: {
        ...
        "crypto_price_queried": "Giá của '{coin_name}' đã được truy vấn thành công",
        "stock_price_queried": "Giá cổ phiếu của '{stock_symbol}' đã được truy vấn thành công"
    },
    ko: {
        ...
        "crypto_price_queried": "'{coin_name}'의 가격이 성공적으로 조회되었습니다",
        "stock_price_queried": "'{stock_symbol}'의 주가가 성공적으로 조회되었습니다"
    },
    it: {
        ...
        "crypto_price_queried": "Il prezzo di '{coin_name}' è stato interrogato con successo",
        "stock_price_queried": "Il prezzo dell'azione '{stock_symbol}' è stato interrogato con successo"
    },
    jv: {
        ...
        "crypto_price_queried": "Rega '{coin_name}' kasil ditakokake",
        "stock_price_queried": "Rega saham kanggo '{stock_symbol}' kasil ditakokake"
    },
    tl: {
        ...
        "crypto_price_queried": "Matagumpay na na-query ang presyo ng '{coin_name}'",
        "stock_price_queried": "Matagumpay na na-query ang presyo ng stock para sa '{stock_symbol}'"
    },
    uz: {
        ...
        "crypto_price_queried": "'{coin_name}' narxi muvaffaqiyatli so'raldi",
        "stock_price_queried": "'{stock_symbol}' aksiya narxi muvaffaqiyatli so'raldi"
    },
    nl: {
        ...
        "crypto_price_queried": "Prijs van '{coin_name}' is succesvol opgevraagd",
        "stock_price_queried": "Aandelenkoers voor '{stock_symbol}' is succesvol opgevraagd"
    },
    el: {
        ...
        "crypto_price_queried": "Η τιμή του '{coin_name}' αναζητήθηκε με επιτυχία",
        "stock_price_queried": "Η τιμή της μετοχής '{stock_symbol}' αναζητήθηκε με επιτυχία"
    },
    sv: {
        ...
        "crypto_price_queried": "Priset på '{coin_name}' har efterfrågats",
        "stock_price_queried": "Aktiekursen för '{stock_symbol}' har efterfrågats"
    },
    he: {
        ...
        "crypto_price_queried": "המחיר של '{coin_name}' נשאל בהצלחה",
        "stock_price_queried": "מחיר המניה של '{stock_symbol}' נשאל בהצלחה"
    },
    da: {
        ...
        "crypto_price_queried": "Prisen på '{coin_name}' blev forespurgt med succes",
        "stock_price_queried": "Aktiekursen for '{stock_symbol}' blev forespurgt med succes"
    },
    fi: {
        ...
        "crypto_price_queried": "Hinta '{coin_name}' kysyttiin onnistuneesti",
        "stock_price_queried": "Osakkeen hinta '{stock_symbol}' kysyttiin onnistuneesti"
    },
    no: {
        ...
        "crypto_price_queried": "Prisen på '{coin_name}' ble spurt",
        "stock_price_queried": "Aksjekursen for '{stock_symbol}' ble spurt"
    }
};
